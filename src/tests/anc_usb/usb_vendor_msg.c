...
static void pc_usb_cmd_set (struct USB_AUDIO_VENDOR_MSG_T* msg)
{
    ...
    s_cur_cmd = cmd_id;
    //ANC_USB_TRACE(3,"%s: cmd[%s], id[%d]", __func__, s_pc_cmd_lst[cmd_id], s_cur_cmd);
	...
	case PC_TOOL_CMD_ANC_MC_EQ_TUNING:
        //ANC_USB_TRACE(1,"recev len:%d",msg->length);
        /*
      	       ANC_USB_TRACE(0,"***********recev test*************");
               for(int i=0;i<msg->length;i++)
               {
                     ANC_USB_TRACE(2,"msg->data[%d]:0x%x",i,msg->data[i]);
               }
               ANC_USB_TRACE(0,"***********recev test*************");
               */
...
{
    //ANC_USB_TRACE(2,"%s: cmd[%d]", __func__, s_cur_cmd);
    ...
    switch (s_cur_cmd) {
        ...
        case PC_TOOL_CMD_SYS_REBOOT:
            //ANC_USB_TRACE(0,"-> cmd_exec: reboot.....");
	...
	case PC_TOOL_CMD_ANC_MC_EQ_TUNING:
               ...
               ANC_USB_TRACE(1,"sned len:%d",msg->length);

/*      	       ANC_USB_TRACE(0,"***********send test*************");
               for(int i=0;i<msg->length;i++)
               {
                    ANC_USB_TRACE(2,"msg->data[%d]:0x%x",i,msg->data[i]);
               }
