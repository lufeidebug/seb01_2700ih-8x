...
void AES_set_key_varible(uint8_t key_len)
{
    if(key_len ==16)
    {
        Nk = 4;
...
void AES256_ECB_encrypt(uint8_t *input, const uint8_t *key, uint8_t *output)
{
    AES_set_key_varible(32);
    // Copy input to output, and work in-memory on output
    BlockCopy(output, input);
    state = ( state_t * )output;
...
void AES256_ECB_decrypt(uint8_t *input, const uint8_t *key, uint8_t *output)
{
    AES_set_key_varible(32);
    // Copy input to output, and work in-memory on output
    BlockCopy(output, input);
    ...
    size_t padding_len)
{
    ...
    *cyphertext_len = 0;
    ...
    if ((NULL == plaintext) || (NULL == cyphertext))
    {
        return false;
    }

    AES128_CBC_encrypt_buffer(cyphertext, plaintext, plaintext_len, key, iv);
    *cyphertext_len = plaintext_len;
