...
static poison_head_t *verify_allocated_region(void *data, bool print_errors)
{
    ...
    if (head->head_canary != HEAD_CANARY_PATTERN) {
        if (print_errors) {
            HEAP_TRACE(1, "CORRUPT HEAP: Bad head at %p. Expected 0x%08x got 0x%08x\n", &head->head_canary,
    ...
    if (canary != TAIL_CANARY_PATTERN) {
        if (print_errors) {
            HEAP_TRACE(1, "CORRUPT HEAP: Bad tail at %p. Expected 0x%08x got 0x%08x\n", &tail->tail_canary,
...
static bool verify_fill_pattern(void *data, size_t size, bool print_errors, bool expect_free, bool swap_pattern)
{
    ...
    if ((intptr_t)data % 4 == 0) {
        ...
        while (size >= 4) {
            if (*p != EXPECT_WORD) {
                if (print_errors) {
                    HEAP_TRACE(1, "CORRUPT HEAP: Invalid data at %p. Expected 0x%08x got 0x%08x\n", p, EXPECT_WORD, *p);
                }
                valid = false;
#ifndef NDEBUG
    ...
    for (int i = 0; i < size; i++) {
        if (p[i] != (uint8_t)EXPECT_WORD) {
            if (print_errors) {
                HEAP_TRACE(1, "CORRUPT HEAP: Invalid data at %p. Expected 0x%02x got 0x%02x\n", p, (uint8_t)EXPECT_WORD, *p);
            }
            valid = false;
#ifndef NDEBUG
...
bool multi_heap_internal_check_block_poisoning(void *start, size_t size, bool is_free, bool print_errors)
{
    if (is_free) {
        ...
        if (head != NULL && head->alloc_size > size - POISON_OVERHEAD) {
            ...
            if (print_errors) {
                HEAP_TRACE(1, "CORRUPT HEAP: Size at %p expected <=0x%08x got 0x%08x\n", &head->alloc_size,
...
{
    poison_head_t *head = (poison_head_t *)((intptr_t)p - sizeof(poison_head_t));
    poison_tail_t *tail = (poison_tail_t *)((intptr_t)p + head->alloc_size);

    assert(head != NULL);

    /*guard address*/
    dwt_enable();
    HEAP_TRACE(0, "%s head %p  guard %p", __func__, p, tail);
    watchpoint_enable(guard, (uint32_t *)&tail->tail_canary,  WPT_WRITE);
}

void multi_heap_disable_tail_guard(multi_heap_handle_t heap, void *p, enum memory_guard guard)
{
    watchpoint_disable(guard);
}
