...
static osTimerId_t svcRtxTimerNew (osTimerFunc_t func, osTimerType_t type, void *argument, const osTimerAttr_t *attr) {
  ...
  if (timer == NULL) {
    ...
    if (timer != NULL) {
      ...
      RTX5_TRACE(2, "TimerInfo: new timer=%p, callback=%p", timer, func);
      RTX5_TRACE(3, "TimerInfo: max_used=%2d, alloc=%2d, free=%2d",
            osRtxTimerMemUsage.max_used, osRtxTimerMemUsage.cnt_alloc, osRtxTimerMemUsage.cnt_free);
    }
#endif
...
static osStatus_t svcRtxTimerDelete (osTimerId_t timer_id) {
  ...
  if ((timer->flags & osRtxFlagSystemObject) != 0U) {
    ...
    RTX5_TRACE(1, "TimerInfo: del timer=%p", timer);
#endif
  }
...
#define OS_TIMER_STATS

#ifdef OS_TIMER_STATS
#include "hal_trace.h"
#include "hal_location.h"
static const char* const timer_states[] = {
    [osRtxTimerInactive] = "Inactive",
    [osRtxTimerStopped] = "Stopped",
    [osRtxTimerRunning] = "Running"
};

static const char* const timer_types[] = {
  [osTimerOnce] = "Oneshot",          ///< One-shot timer.
  [osTimerPeriodic] = "Periodic",          ///< Periodic timer.
};

FLASH_TEXT_LOC
void show_timer_stats(os_timer_t *p)
{
    if(!p)
        return;
    if (p->name)
        REL_TRACE_NOTS(1, "    Timer: %s", p->name);
    else
        REL_TRACE_NOTS(1, "    Timer %p Callback: %p", p, p->finfo.func);

    if (p->state < ARRAY_SIZE(timer_states))
        REL_TRACE_NOTS(1, "    Timer state: %s", timer_states[p->state]);
    if (p->type < ARRAY_SIZE(timer_types))
        REL_TRACE_NOTS(1, "    Timer type %s", timer_types[p->type]);
    REL_TRACE_NOTS(1, "    Timer current tick %u, Load %u\n", p->tick, p->load);
}

FLASH_TEXT_LOC
static void show_handling_timer_stats(os_timer_finfo_t *finfo)
{
    REL_TRACE_NOTS(1, "    Handling Timer callback %p", finfo->func);
}

FLASH_TEXT_LOC
static void rt_timers_scan (void) {
  os_timer_t *p;

  p = osRtxInfo.timer.list;
  while (p != NULL) {
      show_timer_stats(p);
      p = p->next;
  };
...
static uint32_t osTimersHandling (void)
{
    return osMessageQueueGetCount(osRtxInfo.timer.mq);
}

FLASH_TEXT_LOC
void rtx_show_timer_stats(void)
{
    uint32_t handling_timers;


    REL_TRACE_NOTS(1, "\n");
    REL_TRACE_NOTS(0, "--- Timers Stats:\n");
#if (defined(OS_OBJ_MEM_USAGE) && (OS_OBJ_MEM_USAGE != 0))
    REL_TRACE_NOTS(0, "--- Timer Mem: alloc=%d free=%d used=%d",
                        osRtxTimerMemUsage.cnt_alloc, osRtxTimerMemUsage.cnt_free,
                        osRtxTimerMemUsage.max_used);
#endif
    rt_timers_scan();
    REL_TRACE_NOTS(1, "--- Timer scan end\n");

    handling_timers = osTimersHandling();
    /*timers are in the timer thread's queue waiting for processing*/
    if (handling_timers > 0) {
        int i;
        REL_TRACE_NOTS(0, "--- Timers is handleing:%d", handling_timers);

        for (i = 0; i < handling_timers; i++) {
            os_timer_finfo_t finfo;
            osStatus_t       status;

            status = osMessageQueueGet(osRtxInfo.timer.mq, &finfo, NULL, 0);
            if (status == osOK) {
                show_handling_timer_stats(&finfo);
            }
        }
    }
}
