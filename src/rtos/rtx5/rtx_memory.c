...
__STATIC_INLINE void osRtxMemoryStats (void *mem) {
  mem_head_t  *head;

  // Check parameters
  if ((mem == NULL) || (((uint32_t)mem & 7U) != 0U)) {
    return;
  }

  // Initialize memory pool header
  head = MemHeadPtr(mem);
  REL_TRACE_NOTS(0, "--- Memory Size %u, Used %u\n", head->size, head->used);
}

FLASH_TEXT_LOC
void rtx_show_memory_stats(void)
{
  REL_TRACE_NOTS(1, "\n");
  if (osRtxConfig.mem.common_addr != 0U) {
    REL_TRACE_NOTS(0, "--- Memory common Stats:\n");
    osRtxMemoryStats (osRtxConfig.mem.common_addr);
  }
  if (osRtxConfig.mem.stack_addr != 0U) {
    REL_TRACE_NOTS(0, "--- Memory stack Stats:\n");
    osRtxMemoryStats (osRtxInfo.mem.stack);
  }
  if (osRtxConfig.mem.mp_data_addr != 0U) {
    REL_TRACE_NOTS(0, "--- Memory mp_data Stats:\n");
    osRtxMemoryStats (osRtxInfo.mem.mp_data);
  }
  if (osRtxConfig.mem.mq_data_addr != 0U) {
    REL_TRACE_NOTS(0, "--- Memory mq_data Stats:\n");
    osRtxMemoryStats (osRtxConfig.mem.mq_data_addr);
  }
  REL_TRACE_NOTS(0, "--- Memory Stats End:\n");
}
