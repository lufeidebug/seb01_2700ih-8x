...
void rtx_thread_show(const os_thread_t *thread)
{
    ...
    if (thread) {
        ...
        REL_TRACE_NOCRLF_NOTS(0,"--- Thread  %d ", rt_get_TID((const osThreadId_t)thread));
        REL_TRACE_NOCRLF_NOTS(1,"name=%s", thread->name ? thread->name : "NULL");
        REL_TRACE_NOCRLF_NOTS(1," name_mutex=%s ", thread->name_mutex ? thread->name_mutex : "NULL");
        ...
        REL_TRACE_NOTS(3,"    stack_mem=0x%08X stack_size=%u sp:0x%04x min_stack_free=%u",
            (uint32_t)thread->stack_mem, thread->stack_size, thread->sp,
            rtx_thread_get_stack_space(thread));

#ifdef __RTX_CPU_STATISTICS__
        REL_TRACE_NOTS(2,"    swap_in_time=%u(ticks)/%u(ms) swap_out_time=%u(ticks)/%u(ms)",
        ...
        if (thread->swap_in_time <= thread->swap_out_time) {
            ...
            REL_TRACE_NOTS(1,"task runtime %u(ticks)/%u ms",
                                                time, HWTICKS_TO_MS(time));
        } else {
            uint32_t time = rtx_get_hwticks();
            REL_TRACE_NOTS(1,"thread still runing, now %u(ticks)/%u(ms)",
                                                time, HWTICKS_TO_MS(time));
        }
#endif
#endif /*RTX_DUMP_VERBOSE*/

        os_mutex_t *curr_mutex = thread->mutex_list;
        while (curr_mutex) {
            os_mutex_t *next_mutex = curr_mutex->owner_next;
            REL_TRACE_NOTS(3, "    MUTEX: ID=%d NAME=%s FLAG=%d LOCK:%d OWNER=%s",
                           curr_mutex->id, curr_mutex->name, curr_mutex->flags, curr_mutex->lock,
                           curr_mutex->owner_thread->name);
            curr_mutex = next_mutex;
        }
...
static void _rtx_show_thread_usage(const os_thread_t *thread, uint32_t sample_time)
{
    if (thread) {
        if (thread->thread_addr && thread->stack_mem) {
            uint32_t min_free_stack_sz = rtx_thread_get_stack_space(thread);

            REL_TRACE_NOTS(4,"--- Thread name=%s cpu=%%%d min-free-stack=%d",
                    thread->name==NULL ? "null" : (char *)thread->name,
