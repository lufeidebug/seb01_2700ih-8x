...
void osRtxThreadSwitch (os_thread_t *thread) {
  ...
  if (osRtxInfo.thread.run.curr != thread) {
    if (osRtxInfo.thread.run.curr)
      osRtxInfo.thread.run.curr->swap_out_time = rtx_get_hwticks();
    thread->swap_in_time = rtx_get_hwticks();
  }
#endif
...
static uint32_t rt_alloc_TID (void);

/* user thread number + timer thread*/
#define OS_TASK_CNT (OS_TASKCNT + 1)

/* An array of Active task pointers. */
static void *os_active_TCB[OS_TASK_CNT];
...
static osThreadId_t svcRtxThreadNew (osThreadFunc_t func, void *argument, const osThreadAttr_t *attr) {
  ...
  thread->step_rtime      = 0;
    ...
    uint32_t tsk = rt_alloc_TID();
    if (tsk == 0) {
        EvrRtxThreadError(NULL, (int32_t)osError);
        return NULL;
    }
    os_active_TCB[tsk-1] = thread;
    thread->task_id = tsk;
...
static osStatus_t svcRtxThreadTerminate (osThreadId_t thread_id) {
  ...
  if (status == osOK) {
    ...
    uint32_t tid = rt_get_TID(thread);
    if (tid != 0) {
        os_active_TCB[tid-1] = NULL;
        thread->task_id = 0;
    }
...
static uint32_t rt_tsk_self (void) {
  /* Return own task identifier value. */
  osThreadId_t id;
  uint32_t tid;
  id = osRtxThreadGetRunning();
...
uint32_t rt_get_TID (const osThreadId_t id) {
    os_thread_t *thread = (os_thread_t *)id;
