...
archive_bin_path = $(srctree)/$(src)/lib
else ifeq ($(LIB_BIN_IN_TOP_WITH_SRC_DIR),1)
archive_bin_path = $(srctree)/lib/$(src)
else
LIB_BIN_DIR ?= $(BES_LIB_DIR)
# LIB_BIN_DIR might be changed later in directory Makefile.
# archive_bin_path is defined as a recursively expanded variable so that
# everyone can see the new LIB_BIN_DIR value
archive_bin_path = $(srctree)/$(LIB_BIN_DIR)
endif
...
archive_src_target := $(strip $(archive-src-y))

ifeq ($(GEN_LIB),1)
gen_lib_target := $(patsubst $(obj)/%,$(archive_bin_path)/%,$(filter-out $(archive_custom_valid),$(archive_src_target)))
archive_custom_target := $(strip $(archive_custom_valid:$(obj)/%=$(archive_bin_path)/%))
endif

__build: $(builtin-target) $(lib-target) $(buildextra-y) $(buildsubdir-y) \
         $(lst_target) $(archive_src_target) $(gen_lib_target) $(archive_custom_target) \
         $(always)
	@:
...
$(archive-bin-y): $(obj)/%: $(archive_bin_path)/% FORCE
...
ifneq ($(archive_src_target),)

ifneq ($(BESLIB_INFO),)
lib_info_obj = $(obj)/$(basename $(notdir $(1)))_$(BESLIB_INFO).o
endif

define archive_src_target-rule
ifneq ($(BESLIB_INFO),)
$(call lib_info_obj,$(1)) :
	$$(call if_changed,cc_dummy)

$(1): $(call lib_info_obj,$(1))
endif

$(1): $(addprefix $(obj)/,$(patsubst %/,%/$(notdir $(builtin-target)),$($(notdir $(1:.a=-y)))))
endef

$(foreach m, $(archive_src_target), $(eval $(call archive_src_target-rule,$(m))))

      cmd_ar_src_file = ( $(call create_dir,$(dir $@)) ) && $(call archive-cmd,$(filter-out $(PHONY),$^))
quiet_cmd_ar_src_file = AR      $@

$(archive_src_target): FORCE
	$(call if_changed,ar_src_file)

targets += $(archive_src_target)

endif # archive_src_target

ifeq ($(GEN_LIB),1)

ifneq ($(gen_lib_target),)

ifneq ($(CHECK_GEN_LIB_DEP),1)
ALWAYS_PACK_LIB := 1
endif

      cmd_gen_lib_file = ( $(call create_dir,$(dir $@)) ) && $(call CMDCPFILE,$<,$@)
quiet_cmd_gen_lib_file = GENLIB  $@

$(gen_lib_target): $(archive_bin_path)/%: $(obj)/% FORCE
ifeq ($(ALWAYS_PACK_LIB),1)
	$(call cmd,gen_lib_file)
else
	$(call if_changed,gen_lib_file)
endif

targets += $(gen_lib_target)

endif # gen_lib_target

ifneq ($(archive_custom_target),)
targets += $(archive_custom_target)
endif
