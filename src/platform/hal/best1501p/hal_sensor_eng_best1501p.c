...
void hal_sensor_process_rx_buffer(void)
{
    ...
    //HAL_TRACE(2,"dma_buf_active:%08x curr_pos:%08x", dma_buf_active, curr_pos);

    if (dma_buf_active < curr_pos) {
        result_len = dma_rx_buf + dma_rx_len - curr_pos;
        //HAL_TRACE(2,"1:result_len:%08x, curr_pos:%08x", result_len, curr_pos);
        //HAL_DUMP8("%02x ", curr_pos, result_len);
    ...
    if (curr_pos < dma_buf_active) {
        ...
        //HAL_TRACE(2,"2:result_len:%08x, curr_pos:%08x", result_len, curr_pos);
        //HAL_DUMP8("%02x ", curr_pos, result_len);
        if (sensor_eng_handler){
            sensor_eng_handler(HAL_SENSOR_ENGINE_ID_0, HAL_SENSOR_ENGINE_DEVICE_I2C0, curr_pos, result_len);
        }
...
{
    //HAL_TRACE(2,"%s: irq=0x%08x", __func__, sensor_eng[0]->SENSOR_INTR_STS);
...
static void hal_sensor_eng_external_input_pin_irqhandler(enum HAL_GPIO_PIN_T pin)
{
    ...
    HAL_TRACE(2,"hal_sensor_eng_external_irqhandler io=%d val=%d", pin, gpio_lvl);
}

static void sensor_timer_test_handler(enum HAL_I2C_ID_T id, const uint8_t *buf, uint32_t len)
{
    HAL_TRACE(7,"%s: id=%d len=%u %02X-%02X-%02X-%02x", __FUNCTION__, id, len, buf[0], buf[1], buf[2], buf[3]);
}
...
static int sensor_eng_test(enum HAL_SENSOR_ENGINE_TRIGGER_T trigger_type)
{
    ...
    struct HAL_I2C_SENSOR_ENGINE_CONFIG_V1_T se_cfg;
    ...
    if (ret) {
        HAL_TRACE(2,"%s: i2c open failed %d", __func__, ret);
        return ret;
    }
    ...
    while (1) {
        ...
        HAL_TRACE(2,"%s: delay %u ms", __func__, (TRIGGER_TIMER_US / 1000 * LOCK_INTR_CNT));
        hal_sensor_process_rx_buffer();
    }
    int_unlock(lock);
...
int hal_sensor_eng_external_input_test(void)
{
    ...
    HAL_TRACE(5,"SLV0:%08x CFG1:0x%08x CFG3:0x%08x CFG0:0x%08x INTR_MASK:0x%08x",
