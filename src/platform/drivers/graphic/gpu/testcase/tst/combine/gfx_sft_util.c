...
static int util_width = 0;
static int util_height = 0;

#ifndef __rvct__
#include "string.h"
#endif

#define RANDOM_VALUE_INIT   (32557)
static unsigned long int random_value = RANDOM_VALUE_INIT;
#if ! ( defined __LINUX__ || defined __NUCLEUS__  || defined(__QNXNTO__) || defined(__APPLE__) || defined(VGLITE_TST_FIRMWARE))
#include <windows.h>
#include <cassert>
#else
#define BI_RGB        0L
...
static int DATA_STRIDE = 0;
#define LEN 2048
static unsigned char *image_data = NULL;     // Use to change image from RGB to BGR.
static unsigned char *readpixel_data = NULL;

static int DIR_Initialized = FALSE;
static char *MAIN_DIR = NULL;
static char *BMP_DIR = NULL;
static char *LOG_DIR = NULL;
static char *MAIN_LOG = NULL;
static char *INP_DIR = NULL;
...
#if ( defined __LINUX__ || defined __NUCLEUS__ || defined(__QNXNTO__) || defined(VGLITE_TST_FIRMWARE))
...
int InitBMP(int user_fb_width, int user_fb_height)
{
    util_width = user_fb_width;
    util_height = user_fb_height;
    
    InitRects();

    // only support read from screen as RGBA8888 format.
    DATA_STRIDE = util_width * 4;

    image_data = (unsigned char *)MALLOC (util_width*util_height*4 + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER) + 3);
    memset(image_data, 0, util_width*util_height*4 + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER) + 3);

    if(image_data == NULL) {
        VGLITE_TST_PRINTF("\t<<InitBMP:Fail\r\n");
        return TB_BAD;
    }

    readpixel_data = (unsigned char *)(((unsigned int)image_data + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER) + 3) & (~0x3));

    random_value = RANDOM_VALUE_INIT;
            ...
            vg_lite_buffer_format_t format, int stride)
{
    ...
    *((sp = (short *) &infoHeader->biWidth)) = (short)width;
    *((sp = (short *) &infoHeader->biHeight)) = (short)height;
    infoHeader->biPlanes = 1;
    infoHeader->biBitCount = 24;
    *((sp = (short *)&infoHeader->biCompression)) = BI_RGB;
    ...
    for(index = 0; index < height; index++){
        ...
        for (index2 = 0;index2 < width; index2++, l_data_load+=3)
        {
            ...
            case VG_LITE_L8:
                ...
                break;
            case VG_LITE_YUYV:
                VGLITE_TST_PRINTF("not support format!");
                                break;
            default:
                VGLITE_TST_PRINTF("unkonwn format!");
                break;
            }
        }
    }
...
void BMP_File_Name(char *case_name, char *new_name)
{
    ...
    if(strcmp(before_name, case_name) != 0) { // new test cases
        before_name[0] = '\0';
        surf[1] = '0';
        surf[2] = '0';
        surf[3] = '1';
...
//Name: Load_BMP
//Parameters:
//        file_name: The char* type indicate the name of bitmap
//        height: The height of required bitmap
...
{
#if defined (VGLITE_TST_FIRMWARE)
    VGLITE_TST_PRINTF("VGL BAREMETAL: Load_BMP %s %d %d not supported\r\n", file_name, *height, *width);
    return NULL;
#else
...
//Name: ReadBMPFile
//Parameters:
//        file_name: the char* type indicate the file name of bitmap
//        width: the width of required bitmap
//        Height: the height of required bitmap
...
{
#if defined (VGLITE_TST_FIRMWARE)
    VGLITE_TST_PRINTF("VGL BAREMETAL: ReadBMPFile %s %d %d not supported\r\n", file_name, *height, *width);
    return NULL;
#else
...
{
#if defined (VGLITE_TST_FIRMWARE)
    VGLITE_TST_PRINTF("BAREMETAL: SavePNG %s %d not supported\r\n", file_name, i);
    VGL_TST_UNUSED(buf);
#else
...
{
#if defined (VGLITE_TST_FIRMWARE)
    VGLITE_TST_PRINTF("BAREMETAL: WriteBMPFile %s 0x%x %d not supported\r\n", file_name, (uint32_t) pbuf, size);
#else
...

    {
        ...
        {
            VGLITE_TST_PRINTF("errno:%d %s(%d)open file: %s failed!\n",
...
int sft_memcmp(void *buf1, void *buf2, size_t size)
{
    size_t i = 0;
    int num_diffs = 0;
    int offset_1st_diff = 0;
    int got_1st = 0;
    char *b1 = (char *)buf1;
    char *b2 = (char *)buf2;
    int cmp = memcmp(buf1, buf2, size);
    uint32_t *b1_u32_ptr = NULL;
    uint32_t *b2_u32_ptr = NULL;
    
    if (0 == cmp)
    {
        return cmp;
    }
    VGLITE_TST_PRINTF("\t\tsft_memcmp err: b1:b2:s 0x%x:0x%x:0x%x\r\n", (uint32_t)buf1, (uint32_t)buf2, (uint32_t)size);
    ...
    VGLITE_TST_PRINTF("\t\t\t num_diffs:offset_1st 0x%x:0x%x\r\n", (uint32_t)num_diffs, (uint32_t)offset_1st_diff);
    b1_u32_ptr = (uint32_t *)&b1[offset_1st_diff];
    b2_u32_ptr = (uint32_t *)&b2[offset_1st_diff];
    VGLITE_TST_PRINTF("\t\t\t1st uint32 ptrs b1:b2 0x%x:0x%x\r\n", (uint32_t)b1_u32_ptr, (uint32_t)b2_u32_ptr);
    VGLITE_TST_PRINTF("\t\t\t1st uint32 data b1:b2 0x%x:0x%x\r\n", (uint32_t)*b1_u32_ptr, (uint32_t)*b2_u32_ptr);
