...
int InitBMP()
{
    ...
    DATA_STRIDE = (int)WINDSIZEX * 4;

    image_data = (unsigned char *)MALLOC ((int)WINDSIZEX*(int)WINDSIZEY*4 + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER) + 3);
    memset(image_data, 0, (int)WINDSIZEX*(int)WINDSIZEY*4 + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER) + 3);
