...
SFT_CASE SFT_cases[] = 
{
    CASE_ENTRY(SFT_Clear,                        SFT_Clear_Log,                        0),
    CASE_ENTRY(SFT_Path_Draw,                   SFT_Path_Draw_Log,                  2),
    CASE_ENTRY(SFT_Blit,                        SFT_Blit_Log,                        1),
...
vg_lite_error_t Run_SFT()
{
    ...
    while(SFT_cases[i].cases != NULL){
        ...
        if (Cmd == NULL || (specify = (strcmp (SFT_cases[i].case_name, Cmd) == 0)) ){
            ...
            if (error)
            {
                printf("[%s]run case %d failed. error type is %s\n", __func__, __LINE__,error_type[error]);
                return error;
            }
...
vg_lite_error_t Render()
{
    ...
    if (error)
    {
        printf("[%s]Run_SFT %d failed. error type is %s\n", __func__, __LINE__,error_type[error]);
        return error;
    }
