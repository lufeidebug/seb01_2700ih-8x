...
extern vg_lite_buffer_t *sys_fb;
extern int fb_width;
...
int InitBMP()
{
    ...
    DATA_STRIDE = fb_width * 4;
    
    image_data = (unsigned char *)MALLOC (fb_width*fb_height*4 + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER) + 3);
    memset(image_data, 0, fb_width*fb_height*4 + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER) + 3);
