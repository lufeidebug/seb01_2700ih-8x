#ifndef NOSTD
#if defined (TX_IQ_CAL) || defined (RX_IQ_CAL)
#include <math.h>
#include <stdio.h>
...
#define IQ_RANGE_MIN -1000
#define IQ_RANGE_MAX 1000
...
void dc_iq_calib_1503p(void)
{
    ...
    BTDIGITAL_REG_WR(BESMDM_ONE_TONE_SET_GAIN_ADDR, 0x00500012);	//one tone set gain
...
#ifdef IQ_CALIB
    uint32_t en_addr_base = BESMDM_INT_TX_IQCOMP_BASE + 0x8;
    ...
    for (int k = 0; k<3; k++) {
        ...
        float testen=get_iq_energy(en_addr,base_gain,base_phase,1, freq_add);
        ...
        for (int ch = ch1; ch <= ch2; ch++) {
            uint32_t addre=BESMDM_INT_TX_IQCOMP_BASE+ch*4;
    ...
    BTDIGITAL_REG_SET_FIELD(BESMDM_IQMCNTL_ADDR_CT_ADDR, 0x1, 31, 1);
    DRIVERS_TRACE(1,"use time: %d ms", TICKS_TO_MS(hal_sys_timer_get()-time_start));
}
#endif
#endif
