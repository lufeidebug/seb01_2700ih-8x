...
#define CMSIS_NVIC_VIRTUAL_HEADER_FILE "cmsis_nvic_virtual_nuttx.h"
  ...
  #define NVIC_SetPriority(a,b)       NXIC_SetPriority(a+NVIC_USER_IRQ_OFFSET,b)
#else
...
__STATIC_FORCEINLINE void __NVIC_SetVector(IRQn_Type IRQn, uint32_t vector)
{
  ...
  __DSB();
#ifdef MAIN_RAM_USE_SYS_RAM
  SCB_InvalidateICache_by_Addr(&vectors[(int32_t)IRQn + NVIC_USER_IRQ_OFFSET], 4);
#endif
}
 ...
 */
__STATIC_FORCEINLINE uint32_t SCB_GetMVEType(void)
{
