...
extern uint32_t __sync_flags_sram_start[];
extern uint32_t __sync_flags_sram_end[];
...
#define SYNC_FLAG_SIZE                  ((uint32_t)__sync_flags_sram_end - (uint32_t)__sync_flags_sram_start)
#define FLAG_SYNC                       ((uint32_t)__sync_flags_sram_start)
...
static uint32_t Page_4k_Normal_NC;  //outer & inner uncached, non-shareable, executable, rw, domain 0
...
void MMU_CreateTranslationTable(void)
{
...
#ifdef PSRAM_SIZE
    MMU_TTSection (TTB_BASE, PSRAM_BASE             , SECTION_CNT(PSRAM_SIZE)       , Sect_Device_RW);     // multiple of 1MB sections
#endif
#ifdef PSRAMUHS_SIZE
    ...
    //AXI must use DEVICE attribute to read AHB memory
    MMU_TTSection (TTB_BASE, RAM_BASE               , SECTION_CNT(RAM_TOTAL_SIZE)     , Sect_Device_RW);  // multiple of 1MB sections
...
#ifdef GPV_MAIN_BASE
    MMU_TTSection (TTB_BASE, GPV_MAIN_BASE          ,  1, Sect_Device_RW);
#endif
#ifdef GPV_PSRAM1G_BASE
    MMU_TTSection (TTB_BASE, GPV_PSRAM1G_BASE       ,  1, Sect_Device_RW);
#endif
    ...
    // Define synchronization space entry.
    MMU_TTPage4k (TTB_BASE, FLAG_SYNC               , PAGE4K_CNT(SYNC_FLAG_SIZE), Page_L1_4k, (uint32_t *)SYNC_FLAGS_TABLE_L2_BASE_4k, Page_4k_Normal_NC);
