...
/**
 * @brief Handles the PROX interrupt in GPIO IRQ context.
 *
 * Reads the interrupt status register, processes the wearing/removing event and
 * reports the wear state to the upper layer via os_api_callback_proximity.\n
 * If FIFO_FULL is also pending, the INT pin is left uncleared for the FIFO handler.
 *
 * @return true  - FIFO_FULL interrupt is also pending, caller should notify the FIFO task thread.
 * @return false - Only PROX interrupt (or none), INT pin has been cleared in this function.
 */
bool ss_ppg_proximity_interrupt_handler(void);

/**
 * @brief Handles the FIFO_FULL interrupt in task thread context.
 *
 * Reads data from the FIFO, parses and reports PPG data, then clears the INT pin.
 ...
 * Should be called from the FIFO task thread when notified by the IRQ handler
 * in mixed operation mode.
 */
void ss_ppg_interrupt_handler(void);

/**
 * @brief Returns the cached operation mode (RAM read, IRQ-safe).
 *
 * The cache is updated whenever ss_ppg_operation_mode() successfully writes
 * to the sensor register. This allows the GPIO IRQ handler to branch on mode
 * without performing I2C reads (which are forbidden in IRQ context).
 */
