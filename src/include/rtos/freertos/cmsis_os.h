...
static uint64_t os_thread_stack##name[(stacksz > 0)?(((stacksz+7)/8)):1]; \
...
((osTimerDefEx_t*)&pTimer)->os_timer_def.attr.cb_mem = &((osTimerDefEx_t*)&pTimer)->os_timer_cb; \
((osTimerDefEx_t*)&pTimer)->os_timer_def.attr.cb_size = sizeof(StaticTimer_t)
...
typedef struct os_mutex_def_ex  {
  ...
  StaticSemaphore_t  os_mutex_cb;
} osMutexDefEx_t;

#define osMutexInit(pMutex) \
((osMutexDefEx_t*)&pMutex)->os_mutex_def.name = NULL; \
((osMutexDefEx_t*)&pMutex)->os_mutex_def.attr_bits = osMutexRecursive | osMutexPrioInherit; \
((osMutexDefEx_t*)&pMutex)->os_mutex_def.cb_mem = &((osMutexDefEx_t*)&pMutex)->os_mutex_cb; \
((osMutexDefEx_t*)&pMutex)->os_mutex_def.cb_size = sizeof(StaticSemaphore_t)
