...
#define DEFAULT_STACK_SIZE         (WORDS_STACK_SIZE*6)
...
#define CMSIS_V1_SIGNAL_ANY    (0) ///< thread flags: Flags zero, wait for any signal
...
typedef enum  {
  osPriorityIdle          = SCHED_PRIORITY_MIN,                 ///< priority: idle (lowest)
  osPriorityLow           = SCHED_PRIORITY_DEFAULT-2,           ///< priority: low
  osPriorityBelowNormal   = SCHED_PRIORITY_DEFAULT-1,           ///< priority: below normal
  osPriorityNormal        = SCHED_PRIORITY_DEFAULT,             ///< priority: normal (default)
  osPriorityAboveNormal   = SCHED_PRIORITY_DEFAULT+1,           ///< priority: above normal
  osPriorityHigh          = CONFIG_SCHED_HPWORKPRIORITY-1,      ///< priority: high
  osPriorityRealtime      = SCHED_PRIORITY_MAX-2,               ///< priority: realtime (highest)
  osPriorityNone          =  0,                                 ///< No priority (not initialized).
  osPriorityError         =  0xff,                              ///< system cannot determine priority or thread has illegal priority
  osPriorityISR           =  254,
} osPriority;
...
#define osPriority_t osPriority
#define osTimerType_t os_timer_type
...
typedef struct os_timer_def  {
  ...
  osTimerAttr_t             attr;    ///< pointer to internal data
  os_timer_type             type;
  void                      *timerid;
  void                      *argument;
} osTimerDef_t;
...
((osTimerDefEx_t*)&pTimer)->os_timer_def.attr = {0}; \
((osTimerDefEx_t*)&pTimer)->os_timer_def.type = osTimerOnce; \
((osTimerDefEx_t*)&pTimer)->os_timer_def.timerid = (timer_t)0; \
((osTimerDefEx_t*)&pTimer)->os_timer_def.argument = 0; \
...
typedef osThreadDef_t *osThreadId_t;

/// \details Timer ID identifies the timer.
typedef osTimerDef_t *osTimerId_t;

/// \details Event Flags ID identifies the event flags.
typedef void *osEventFlagsId_t;

/// \details Mutex ID identifies the mutex.
typedef osMutexDef_t *osMutexId_t;

/// \details Semaphore ID identifies the semaphore.
typedef osSemaphoreDef_t *osSemaphoreId_t;
...
typedef osThreadDef_t *osThreadId;

/// Timer ID identifies the timer (pointer to a timer control block).
/// \note CAN BE CHANGED: \b os_timer_cb is implementation specific in every CMSIS-RTOS.
typedef osTimerDef_t *osTimerId;

/// Mutex ID identifies the mutex (pointer to a mutex control block).
/// \note CAN BE CHANGED: \b os_mutex_cb is implementation specific in every CMSIS-RTOS.
typedef osMutexDef_t *osMutexId;

/// Semaphore ID identifies the semaphore (pointer to a semaphore control block).
/// \note CAN BE CHANGED: \b os_semaphore_cb is implementation specific in every CMSIS-RTOS.
typedef osSemaphoreDef_t *osSemaphoreId;

/// Pool ID identifies the memory pool (pointer to a memory pool control block).
/// \note CAN BE CHANGED: \b os_pool_cb is implementation specific in every CMSIS-RTOS.
typedef osPoolDef_t * osPoolId;
...
typedef osMailQDef_t *osMailQId;
...
{ 0,priority,0,stacksz,0,0,name,0,task_name,0}
#endif
...
osThreadId osThreadCreateExt(const osThreadDef_t *thread_def, void *argument, int arg2);
...
/* cmsis interface thread name */
const char *osThreadGetName (osThreadId id);
...
#else                            // define the object
#define osTimerDef(name, function)  \
const osTimerAttr_t os_timer_attr_##name = {# name, 0 ,0, 0}; \
const osTimerDef_t os_timer_def_##name = \
{ (function), (os_timer_attr_##name) ,osTimerOnce}
#endif
...
{ 0,0,(queue_sz), sizeof(type), 0,(_TO_STRING(name)) }
#endif
