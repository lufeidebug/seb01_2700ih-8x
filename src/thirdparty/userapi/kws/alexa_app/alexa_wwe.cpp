...
{
    USERAPI_TRACE(2,"%s %d", __func__, prlBinaryModelLen);
...
static int alexa_wwe_lib_init(AlexaCallback cb, int ch)
{
    ...
    for (i = 0; i < AlexaCh; i++)
    {
        ...
        USERAPI_TRACE(3,"%s requiredMem %d err %d", __func__, configAttributes.requiredMem, err);   //23344

        if (err != PRYON_LITE_ERROR_OK)
        {
            // handle error
            USERAPI_TRACE(3,"!!!!!!!!!!%s %d err %d", __func__, __LINE__, err);
            return -1;
        }
...
static bool wake_word_event_handle(void *param, char ch)
{
    ...
    USERAPI_TRACE(3,"%s score %d ch %d", __func__, wake_word_->score, ch);
    USERAPI_TRACE(2,"len %d end %d", len_of_queue, 2*wake_word_->end_index);
    if (alexa_wwe_ai_connected && !alexa_wwe_ai_in_sco)
    {
        if (len_of_queue > 2*wake_word_->end_index)
        {
            ...
            USERAPI_TRACE(1,"len_of_delete %d", len_of_delete);
...
static int alexa_wwe_init(bool init, const void *)
{
    ...
    if (NULL == alexa_thread_tid)
    {
        USERAPI_TRACE(0,"Failed to Create alexa_thread\n");
    }
...
static uint32_t far_field_capture_callback(uint8_t *buf, uint32_t len)
{
    ...
    {
        EnCQueue_AI(&alexa_data_buf_queue, buf, len, NULL);
    }
    else
    {
        EnCQueue_AI(&alexa_data_out_buf_queue, buf, len, NULL);
    }
    osSignalSet(alexa_thread_tid, ALEXA_SIGNAL_ID);
    return len;
}
...
static int alexa_wwe_power_on_init(bool on, const void *)
{
    ...
    USERAPI_TRACE(6,"%s %p %p %p %p %p", __func__, 
                alexa_data_buf,
                frame_data_buf,
                alexa_data_out_buf,
                engineBuffer[0],
...
static int alexa_wwe_stream_start(bool on, const void *)
{
    ...
    USERAPI_TRACE(4,"%s work:%d op:%d init:%d", __func__, isRun, on, alexa_wwe_initated);
    if (!alexa_wwe_initated)
    {
        on = false;
        USERAPI_TRACE(1,"%s alexa_wwe don't initated", __func__);
        return 0;
    }

    if (!alexa_wwe_ai_connected)
    {
        USERAPI_TRACE(1,"%s alexa_wwe ai don't connect", __func__);
        return 0;
    }

    if (isRun == on)
    {
        USERAPI_TRACE(1,"%s isRun == on", __func__);
        return 0;
    }
    ...
    if (on)
    {
        ...
        // USERAPI_TRACE(1,"sys freq calc: %d Hz", hal_sys_timer_calc_cpu_freq(5, 0));
        USERAPI_TRACE(2,"alexa_data_buf %p alexa_data_out_buf %p", alexa_data_buf, alexa_data_out_buf);
        ...
        USERAPI_TRACE(2,"codec capture sample_rate:%d, data_size:%d",stream_cfg.sample_rate,stream_cfg.data_size);
...
{
    USERAPI_TRACE(2,"%s ai connect %d",__func__, alexa_wwe_ai_connected);
    if (alexa_wwe_ai_connected && !alexa_wwe_ai_in_sco)
    {
...
static void alexa_wwe_provide_speech_handle(void)
{
    ...
    {
        len_of_queue = LengthOfCQueue(&alexa_data_out_buf_queue);
        USERAPI_TRACE(2,"%s len %d", __func__, len_of_queue);
        if (len_of_queue)
        {
...
{
    USERAPI_TRACE(2,"%s wake %d",__func__, wake_word_event_comed);
...
static int alexa_wwe_ai_connect(bool status, void *param)
{
    ...
    USERAPI_TRACE(3,"%s ai connect %d sco %d", __func__, alexa_wwe_ai_connected, alexa_wwe_ai_in_sco);
    ...
    while(1)
    {
        ...
        if(evt.status == osEventSignal)
        {
            if (!wake_word_event_comed)
            {
                ...
                {
                    USERAPI_TRACE(1,"%s wake_word_event_now", __func__);
                    wake_word_event_now = false;
                ...
                {
                    USERAPI_TRACE(1,"%s wake_word_provide_speech", __func__);
                ...
                else if (LengthOfCQueue(&alexa_data_buf_queue) >= WWE_HANDLE_FRAME_SIZE_LEN)
                {
                    ...
                    EnCQueue_AI(&alexa_data_out_buf_queue, frame_data_buf, WWE_HANDLE_FRAME_SIZE_LEN, NULL);
                    alexa_audio_sample_input((short *)frame_data_buf, WWE_HANDLE_FRAME_SIZE_LEN/2, 1);
                    //etime=hal_sys_timer_get();
                    //USERAPI_TRACE(1,"ALEXA COST TIME %d", TICKS_TO_MS(etime-stime));
                    //USERAPI_TRACE(2,"%s ====>sys freq calc : %d\n", __func__, hal_sys_timer_calc_cpu_freq(50, 0));
                }
            }
            else
            {
                ...
                {
                    USERAPI_TRACE(1,"%s wake_word_event_stop", __func__);
                    wake_word_event_stop = false;
                    app_thirdparty_callback_handle(THIRDPARTY_STOP_SPEECH_CALLBACK, NULL, 0);
                    alexa_wwe_restart();
                    wake_word_event_comed = false;
                ...
                else if (LengthOfCQueue(&alexa_data_out_buf_queue) >= AI_SENT_FRAME_SIZE_LEN)
                {
                    ...
                    {
                        USERAPI_TRACE(2,"%s len %d", __func__, LengthOfCQueue(&alexa_data_out_buf_queue));
                        stime=hal_sys_timer_get();
                        ...
                        etime=hal_sys_timer_get();
                        USERAPI_TRACE(1,"OPUS COST TIME %d", TICKS_TO_MS(etime-stime));
                        //osDelay(5);
                        //USERAPI_TRACE(2,"%s ====>sys freq calc : %d\n", __func__, hal_sys_timer_calc_cpu_freq(50, 0));
                    }
                }
...
{
    {TP_EVENT(THIRDPARTY_ID_ALEXA_WWE, THIRDPARTY_INIT,              alexa_wwe_power_on_init),   false,  NULL},
    {TP_EVENT(THIRDPARTY_ID_ALEXA_WWE, THIRDPARTY_AI_PROVIDE_SPEECH, alexa_wwe_provide_speech),  false,  NULL},
    {TP_EVENT(THIRDPARTY_ID_ALEXA_WWE, THIRDPARTY_AI_STOP_SPEECH,    alexa_wwe_stop_speech),     false,  NULL},
    {TP_EVENT(THIRDPARTY_ID_ALEXA_WWE, THIRDPARTY_AI_CONNECT,        alexa_wwe_ai_connect),      false,  NULL},
    {TP_EVENT(THIRDPARTY_ID_ALEXA_WWE, THIRDPARTY_AI_DISCONNECT,     alexa_wwe_ai_disconnect),   false,  NULL},
