...
#define SNDP_NAME           soundplus
#define BES_AQE_NAME        aqe
...
typedef enum
{
    ...
    THIRDPARTY_ID_BES_AQE,          // bes kws aqe
    THIRDPARTY_ID_SNDP,            // Sndp wakeup word engine
    THIRDPARTY_ID_KWS_ENGINE,
    THIRDPARTY_ID_NT_DEMO,
    THIRDPARTY_ID_KNOWLES,
...
typedef enum
{
    ...
    /// stop upload voice data to APP, should start local detection again
    ...
    THIRDPARTY_MIC_DATA_COME    = 0x15,
    THIRDPARTY_HW_DETECT_START  = 0x16,
    THIRDPARTY_HW_DETECT_STOP   = 0x17,
    THIRDPARTY_EVENT_NUMBER,
}THIRDPARTY_EVENT_TYPE;

typedef enum
{
    THIRDPARTY_WAKE_UP_CALLBACK = 0x0,
    THIRDPARTY_DATA_COME_CALLBACK,
...
#define TP_EVENT(lib_id, event, handler) {THIRDPARTY_FUNC_TYPE, lib_id, event}, (APP_THIRDPARTY_HANDLE_CB_T)handler
...
EXTERN_THIRDPARTY_HANDLER_TAB_AND_SIZE(SNDP_NAME)
EXTERN_THIRDPARTY_HANDLER_TAB_AND_SIZE(BES_AQE_NAME)
...
int app_thirdparty_specific_lib_event_handle(THIRDPARTY_FUNC_ID funcId,THIRDPARTY_EVENT_TYPE event_type, uint8_t ai_spec);
