...
uint32_t ai_get_ai_cmd_transport_fifo_length(void)
{
    return kfifo_len(&ai_cmd_transport_fifo);
}
...
void LOCK_AI_TRANSPORT_FIFO(void)
{
    if(osMutexWait(ai_transport_fifo_mutex_id, osWaitForever) != osOK)
        AI_VOICE_TRACE(1,"%s Error", __func__);
}
...
void ai_transport_fifo_clean(void)
{
    memset(ai_rev_buf, 0, sizeof(ai_rev_buf));
    memset(ai_cmd_transport_fifo_buff, 0, sizeof(ai_cmd_transport_fifo_buff));
}
...
bool ai_transport_cmd_value(void)
{
    ...
    if (total_len <= AI_CMD_TRANSPORT_BUFF_FIFO_SIZE)
    {
        ...
        {
            //AI_VOICE_DUMP8("%x ", cmd_buf, total_len);
    ...
    AI_VOICE_TRACE(4,"%s cmd len %d total %d count_time %d", __func__, cmd_len, total_len, count_time);
...
{
    bool isCmdPutSuccessfully = ai_transport_cmd_put_generic(in_buf, len, false);
    return isCmdPutSuccessfully;
}

bool ai_transport_cmd_put_generic(uint8_t *in_buf, uint16_t len, bool isSkipWhenFifoOverflow)
{
    unsigned int freeSpace = 0;
    AI_VOICE_TRACE(3,"%s len=%d buf=%p", __func__, len, in_buf);
    ...
    if (in_buf==NULL)
    {
        AI_VOICE_TRACE(1,"%s the buf is null", __func__);
        return false;
    }

    LOCK_AI_TRANSPORT_FIFO();
    if (isSkipWhenFifoOverflow)
    {
        freeSpace = kfifo_get_free_space(&ai_cmd_transport_fifo);
        if (freeSpace < len)
        {
            AI_VOICE_TRACE(1,"kfifo overflow!!! free space = %d", freeSpace);
            UNLOCK_AI_TRANSPORT_FIFO();
            return false;
        }
        else
        {
            kfifo_put(&ai_cmd_transport_fifo, in_buf, len);
        }
    }
    else
    {
        freeSpace = kfifo_put(&ai_cmd_transport_fifo, in_buf, len);
        if (freeSpace != len)
        {
            ASSERT(false, "%s: AI transport fifo OVERFLOW len %d freeSpace %d", __func__, len, freeSpace);
        }
    }
...
static bool ai_transport_cmd(uint8_t ai_index, uint8_t dest_id)
{
    ...
    {
        AI_VOICE_TRACE(0,"cmd fifo is error, need init");
        LOCK_AI_TRANSPORT_FIFO();
        kfifo_init(&ai_cmd_transport_fifo, ai_cmd_transport_fifo_buff, sizeof(ai_cmd_transport_fifo_buff));
        UNLOCK_AI_TRANSPORT_FIFO();
        goto cmd_trans_end;
    }
    while (kfifo_len(&ai_cmd_transport_fifo))
    {
        LOCK_AI_TRANSPORT_FIFO();
        ...
        if (cmd_len > app_ai_get_mtu(ai_index))
        {
            ...
            AI_VOICE_TRACE(2, "cmd_len %d too large, mtu is %d", cmd_len, app_ai_get_mtu(ai_index));
            goto cmd_trans_end;
        }

        if(!app_ai_get_connected_num())
        {
            AI_VOICE_TRACE(0,"error ai disconnect");
            kfifo_get(&ai_cmd_transport_fifo, (uint8_t *)&ai_send_cmd_t, cmd_len+AI_CMD_CODE_SIZE);
            goto cmd_trans_end;
        }

        if(0 == (GET_DEST_FLAG(dest_id)) && (ai_trans_handler[ai_index].ai_spp_cmd_transport_func != NULL))
        {
            ret = ai_trans_handler[ai_index].ai_spp_cmd_transport_func(ai_send_cmd_t.data, cmd_len, ai_index, dest_id);
        }
        else if (ai_trans_handler[ai_index].ai_ble_cmd_transport_func != NULL)
        {
...
cmd_trans_end:
        if (ret == true)
        {
            AI_VOICE_TRACE(1, "%s ok %d %d", __func__, cmd_len+AI_CMD_CODE_SIZE, kfifo_len(&ai_cmd_transport_fifo));
            kfifo_get(&ai_cmd_transport_fifo, (uint8_t *)&ai_send_cmd_t, cmd_len+AI_CMD_CODE_SIZE);
        }
        else
        {
            AI_VOICE_TRACE(1, "%s fail", __func__);
        }
    }
...
static bool ai_transport_data(uint8_t ai_index, uint8_t dest_id)
{
    ...
    {
        AI_VOICE_TRACE(1,"WARN:%s don't allow", __func__);
        goto data_trans_end;
    }
    ...
    {
        AI_VOICE_TRACE(2,"%s data_len %d", __func__, data_len);
        goto data_trans_end;
    }
    ...
    if (send_len > app_ai_get_mtu(ai_index))
    {
        ...
        AI_VOICE_TRACE(2, "send_len %d too large, mtu is %d", send_len, app_ai_get_mtu(ai_index));
        goto data_trans_end;
    }

    AI_VOICE_TRACE(3,"%s data len %d send len %d Qlen = %d", __func__, data_len, send_len,voice_compression_get_encode_buf_size());

    if (0 == (GET_DEST_FLAG(dest_id)) && (ai_trans_handler[ai_index].ai_spp_data_transport_func != NULL))
    {
        ret = ai_trans_handler[ai_index].ai_spp_data_transport_func(ai_send_cmd_t.data, send_len, ai_index, dest_id);
    }
    else if (ai_trans_handler[ai_index].ai_ble_data_transport_func != NULL)
    {
...
data_trans_end:
    ...
    {
        AI_VOICE_TRACE(1, "ERR:%s fail", __func__);
    }
    return ret;
}
...
bool ai_transport_handler(uint8_t ai_index, uint8_t dest_id)
{
    ...
    if ((0 == ai_struct[ai_index].tx_credit) && (0 != GET_DEST_FLAG(dest_id)))
    {
        AI_VOICE_TRACE(2,"%s no txCredit ai_index=%d", __func__, ai_index);
        goto trans_end;
    }

    if (ai_transport_is_busy(ai_index))
    {
        AI_VOICE_TRACE(1,"%s is busy now", __func__);
        goto trans_end;
    }
    ...
    if ((true == ret)  && (0 != GET_DEST_FLAG(dest_id)))
    {
