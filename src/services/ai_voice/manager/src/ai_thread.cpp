...
void app_ai_connect_info_init(void)
{
    ...
    for (ai_connect_index = 0; ai_connect_index < AI_CONNECT_NUM_MAX; ai_connect_index++)
    {
        ...
        ai_connect_info[ai_connect_index].connhdl = BT_INVALID_CONN_HANDLE;
        memset(ai_connect_info[ai_connect_index].addr, 0, 6);
    }
}
#ifdef __IAG_BLE_INCLUDE__
uint8_t app_ai_set_ble_connect_info(uint8_t type, uint8_t aiSpec, bes_ble_ai_event_param_t *param)
{
    uint8_t ai_connect_index = 0;

    for (ai_connect_index = 0; ai_connect_index < AI_CONNECT_NUM_MAX; ai_connect_index++)
    {
        if(ai_connect_info[ai_connect_index].ai_connect_state != AI_IS_CONNECTED)
        {
            ...
            ai_connect_info[ai_connect_index].conidx = param->conidx;
            ai_connect_info[ai_connect_index].connhdl = param->connhdl;
            break;
        }
    }
...
{
    AI_VOICE_TRACE(3, "%s conIdx:%d, state:%d", __func__, ai_connect_index, state);
...
uint8_t app_ai_get_connect_index_from_ble_conidx(uint8_t conidx, uint8_t ai_spec)
{
    ...
    for (ai_connect_index = 0; ai_connect_index < AI_CONNECT_NUM_MAX; ai_connect_index++)
    {
        if(ai_connect_info[ai_connect_index].conidx == conidx && ai_connect_info[ai_connect_index].ai_spec == ai_spec)
        {
            ...
            {
                AI_VOICE_TRACE(2, "%s %d", __func__, ai_connect_index);
                return ai_connect_index;
            }
        }
    }
...
uint16_t app_ai_get_conn_handle_from_ble_conidx(uint8_t conidx, uint8_t ai_spec)
{
    uint8_t ai_connect_index = 0;
    ...
    for (ai_connect_index = 0; ai_connect_index < AI_CONNECT_NUM_MAX; ai_connect_index++)
    {
        if(ai_connect_info[ai_connect_index].conidx == conidx && ai_connect_info[ai_connect_index].ai_spec == ai_spec)
        {
            if(ai_connect_info[ai_connect_index].ai_connect_type == AI_TRANSPORT_BLE)
            {
                AI_VOICE_TRACE(0, "%s %d %04x", __func__, ai_connect_index, ai_connect_info[ai_connect_index].connhdl);
                return ai_connect_info[ai_connect_index].connhdl;
            }
        }
    }
...
uint8_t app_ai_get_connect_index(uint8_t device_id, uint8_t ai_spec)
{
    uint8_t connect_index;
    if (0 == (GET_DEST_FLAG(device_id))) {
        connect_index = app_ai_get_connect_index_from_device_id(device_id, ai_spec);
    } else {
        connect_index = app_ai_get_connect_index_from_ble_conidx(GET_BLE_CONIDX(device_id), ai_spec);
    }
...
uint8_t app_ai_set_spp_connect_info(uint8_t *_addr, uint8_t device_id, uint8_t type, uint8_t AiSpec)
{
    ...
    for (ai_connect_index = 0; ai_connect_index < AI_CONNECT_NUM_MAX; ai_connect_index++)
    {
        if (ai_connect_info[ai_connect_index].ai_spec == AiSpec || (ai_connect_info[ai_connect_index].ai_connect_state) != AI_IS_CONNECTED)
        {
            ai_connect_info[ai_connect_index].ai_connect_state = AI_IS_CONNECTED;
            ai_connect_info[ai_connect_index].ai_connect_type = type;
            ai_connect_info[ai_connect_index].ai_spec = AiSpec;
            ...
            if (_addr)
            {
                ...
                AI_VOICE_DUMP8("%02x ", _addr, BT_ADDR_OUTPUT_PRINT_NUM);
            }
            break;
        }
    }

    AI_VOICE_TRACE(3, "%s connIdx:%d <--> ai_spec:%d", __func__, ai_connect_index, AiSpec);
    return ai_connect_index;
}
...
uint8_t app_ai_get_connected_num(void)
{
...
bool app_ai_check_device_is_connected(uint8_t *_addr)
{
    bool ret = false;
    for (uint8_t ai_connect_index = 0; ai_connect_index < AI_CONNECT_NUM_MAX; ai_connect_index++)
    {
        if (!memcmp(ai_connect_info[ai_connect_index].addr, _addr, 6) && \
                    (ai_connect_info[ai_connect_index].ai_connect_state == AI_IS_CONNECTED))
        {
            ret = true;
            break;
...
uint8_t app_ai_get_dest_id_by_ai_spec(uint8_t ai_spec)
{
    ...
    for(uint8_t i=0; i<AI_CONNECT_NUM_MAX; i++)
    {
        ...
        {
            dest_id = app_ai_get_dest_id(&(ai_connect_info[i]));
            break;
        }
    }
    return dest_id;
...
{
    AI_VOICE_TRACE(2, "%s ai %d", __func__, ai_connect_index);
...
uint8_t app_ai_get_connect_index_from_addr(uint8_t *_addr, uint8_t aiSpec)
{
    uint8_t ai_connect_index = 0;
...
uint8_t app_ai_get_ai_index_from_connect_index(uint8_t connect_index)
{
    ...
    {
        if (ai_connect_info[connect_index].ai_spec == ai_struct[ai_index].currentAiSpec)
        {
...
{
    //AI_VOICE_TRACE(3,"%s %d%s", __func__, ai_index, ai_spec_type2str((AI_SPEC_TYPE_E)ai_index));
    ai_struct[ai_index].currentAiSpec = ai_index;
}

uint8_t app_ai_get_ai_spec(uint8_t ai_index)
{
    //AI_VOICE_TRACE(4,"%s %d%s", __func__, ai_struct[ai_index].currentAiSpec, ai_spec_type2str((AI_SPEC_TYPE_E)ai_struct[ai_index].currentAiSpec));
...
{
    AI_VOICE_TRACE(3,"%s ai %d index %d type %d%s", __func__, ai_index, ai_connect_index, type, ai_trans_type2str(type));
...
{
    /*
    AI_VOICE_TRACE(3,"%s ai %d type %d%s", __func__,
                     ai_index,
...
{
    AI_VOICE_TRACE(2,"%s %d ai %d", __func__, trans_size, ai_index);
    if (app_ai_get_mtu(ai_index) != AI_TRANS_DEINIT_MTU_SIZE &&
            trans_size > app_ai_get_mtu(ai_index))
    {
        AI_VOICE_TRACE(2,"%s error mtu %d", __func__, app_ai_get_mtu(ai_index));
        trans_size = app_ai_get_mtu(ai_index);
    }
...
uint8_t app_ai_read_tx_credit(uint8_t ai_index)
{
    if (ai_index < AI_SPEC_COUNT)
    {
        return ai_struct[ai_index].tx_credit;
    }
    else
    {
        AI_VOICE_TRACE(1,"%s Invaild ai_index!!!", __func__);
        return 0xFF;
    }
}

bool app_ai_take_tx_credit(uint8_t ai_index)
{
    if (ai_index < AI_SPEC_COUNT)
    {
        if (ai_struct[ai_index].tx_credit > 0)
        {
    ...
    {
        AI_VOICE_TRACE(1,"%s fail due to invaild ai_index!!!", __func__);
        return false;
    }
}

bool app_ai_release_tx_credit(uint8_t ai_index)
{
    if (ai_index < AI_SPEC_COUNT)
    {
        if (ai_struct[ai_index].tx_credit < MAX_TX_CREDIT)
        {
            ai_struct[ai_index].tx_credit++;
        }
        return true;
...
void app_ai_set_mtu(uint32_t mtu, uint8_t ai_index)
{
    ...
    AI_VOICE_TRACE(2,"%s %d t_size %d ai %d", __func__, mtu, trans_size, ai_index);
    if (trans_size && trans_size > mtu)
    {
        AI_VOICE_TRACE(2,"%s error trans size %d", __func__, trans_size);
...
{
    AI_VOICE_TRACE(2,"%s %d", __func__, type);
...
{
    AI_VOICE_TRACE(3,"%s %d%s", __func__, speech_state, ai_speech_state2str(speech_state));
    ai_struct[ai_index].speechState = speech_state;
}

APP_AI_SPEECH_STATE_E app_ai_get_speech_state(uint8_t ai_index)
{
    AI_VOICE_TRACE(3,"%s %d%s", __func__,
...
{
    AI_VOICE_TRACE(2,"%s size %d", __func__, sizeof(ai_struct[ai_index].ai_mobile_info));
    ...
    for (ai_index = 0; ai_index < MUTLI_AI_NUM; ai_index++)
    {
        for (index=0; index<BT_DEVICE_NUM; index++)
        {
            if (ai_struct[ai_index].ai_mobile_info[index].used == true)
            {
                ...
                {
                    AI_VOICE_TRACE(2, "%s %d", __func__, index);
                    break;
                }
            }
        }
...
static void app_ai_set_mobile_info(MOBILE_CONN_TYPE_E type, uint8_t *_addr)
{
    ...
    index = app_ai_get_mobile_index(_addr);
    if (index < BT_DEVICE_NUM)
    {
        AI_VOICE_TRACE(1,"%s already save the info", __func__);
        for (ai_index = 0; ai_index < MUTLI_AI_NUM; ai_index++)
        {
        ...
        for (ai_index = 0; ai_index < MUTLI_AI_NUM; ai_index++)
        {
            for (index=0; index<BT_DEVICE_NUM; index++)
            {
                if (ai_struct[ai_index].ai_mobile_info[index].used == false)
                {
                    ...
                    //AI_VOICE_TRACE(5,"%s type %d%s index %d ai_index %d addr:", __func__, type, mobile_connect_type2str(type), index, ai_index);
                    //AI_VOICE_DUMP8("%02x ", _addr->address, BT_ADDR_OUTPUT_PRINT_NUM);
                    break;
                }
            }
        }
...
static uint8_t app_ai_clear_mobile_info(uint8_t *_addr)
{
    ...
    AI_VOICE_TRACE(2,"%s index %d addr:", __func__, index);
    AI_VOICE_DUMP8("%02x ", _addr, BT_ADDR_OUTPUT_PRINT_NUM);

    if (index >= BT_DEVICE_NUM)
    {
        AI_VOICE_TRACE(2,"%s don't have this addr info, index=%d", __func__, index);
    }
    else
    {
        for (ai_index = 0; ai_index < MUTLI_AI_NUM; ai_index++)
...
void app_ai_disconnect_all_mobile_link(void)
{
    ...
    for(uint8_t ai_index = 0; ai_index < MUTLI_AI_NUM; ai_index++)
    {
        for (uint8_t index=0; index < BT_DEVICE_NUM; index++)
        {
            if (ai_struct[ai_index].ai_mobile_info[index].used == true)
            {
                AI_VOICE_TRACE(1,"disconnect bt index %d addr:", index);
                AI_VOICE_DUMP8("%02x ", ai_struct[ai_index].ai_mobile_info[index].addr, BT_ADDR_OUTPUT_PRINT_NUM);
                handle = bts_bt_if_get_dev_acl_handle((bt_bdaddr_t *)ai_struct[ai_index].ai_mobile_info[index].addr);
                btif_me_force_disconnect_link_with_reason(handle, BTIF_BEC_USER_TERMINATED, TRUE);
            }
        }
    }
}
...
{
    AI_VOICE_TRACE(3,"%s %d%s", __func__, type, ai_speech_type2str(type));
...
{
    AI_VOICE_TRACE(2,"%s spec:%d, codec:%d", __func__, ai_index, encode_type);
    ai_struct[ai_index].encode_type = encode_type;
}

uint8_t app_ai_get_encode_type(uint8_t ai_index)
{
    //AI_VOICE_TRACE(2,"%s %d", __func__, ai_struct[ai_index].encode_type);
...
void ai_set_can_wake_up(bool wake_up, uint8_t ai_index)
{
    ...
    {
        AI_VOICE_TRACE(2, "AI(%d) can wakeup update:%d->%d", ai_index, ai_struct[ai_index].can_wake_up, wake_up);
...
void ai_audio_stream_allowed_to_send_set(bool isEnabled, uint8_t ai_index)
{
    ...
    {
        AI_VOICE_TRACE(2, "AI(%d) allow to send update:%d->%d",
...
{
    AI_VOICE_TRACE(2, "AI(%d) stream init state update:%d->%d",
...
{
    AI_VOICE_TRACE(3, "Stream running state for spec[%d] update:%d->%d",
...
void app_ai_set_algorithm_engine_reset(bool reset, uint8_t ai_index)
{
    ...
    {
        AI_VOICE_TRACE(2, "Engine reset flag for AI(%d) update:%d->%d", 
...
{
    AI_VOICE_TRACE(2,"%s %d", __func__, use_ai_32kbps_voice);
...
{
    AI_VOICE_TRACE(2,"%s %d", __func__, local_start_tone);
...
{
    AI_VOICE_TRACE(2,"%s %d index %d", __func__, is_in_tws_mode, ai_index);
...
bool app_ai_is_enabled_wakeword(uint8_t ai_index)
{
    return ai_struct[ai_index].enabled_kws;
}

void app_ai_enable_wakeword(bool enable, uint8_t ai_index)
{
    if(enable != ai_struct[ai_index].enabled_kws)
    {
        ai_struct[ai_index].enabled_kws = enable;
        // make sure not in speeching now
        app_ai_set_use_thirdparty(enable, ai_index);
    }
}

void app_ai_set_use_thirdparty(bool is_use_thirdparty, uint8_t ai_index)
{
    AI_VOICE_TRACE(3,"%s ai:%d use_thirdparty:%d", __func__, ai_index, is_use_thirdparty);
    ...
    if (app_ai_is_setup_complete(ai_index))
    {
        ...
        if (is_ai_can_wake_up(ai_index))
        {
            if (is_use_thirdparty == false && \
                app_ai_is_use_thirdparty(ai_index))
            {
                ...
                !app_ai_is_use_thirdparty(ai_index) && \
                app_ai_is_enabled_wakeword(ai_index))
            {
                #ifdef __THIRDPARTY
...
void app_ai_set_is_do_encoding_during_detecting(bool isEnable, uint8_t ai_index)
{
    ai_struct[ai_index].isDoEncodingOnDetecting = isEnable;
}

bool app_ai_is_do_encoding_during_detecting(uint8_t ai_index)
{
    return ai_struct[ai_index].isDoEncodingOnDetecting;
}

void app_ai_check_if_need_set_thirdparty(uint8_t ai_index)
{
    AI_VOICE_TRACE(2,"%s %d %d", __func__, ai_struct[ai_index].want_to_use_thirdparty, ai_struct[ai_index].is_use_thirdparty);
...
{
    AI_VOICE_TRACE(2,"%s %d", __func__, opus_in_overlay);
...
void app_ai_mobile_connect_handle(MOBILE_CONN_TYPE_E type, uint8_t *_addr)
{
    ...
    //{
    //    AI_VOICE_TRACE(1, "%s set ai connect", __func__);
...
void app_ai_mobile_disconnect_handle(uint8_t *_addr)
{
    ...
    if (index >= BT_DEVICE_NUM)
    {
        AI_VOICE_TRACE(1, "%s can't clear this mobile info", __func__);
        return;
    }
#ifndef FREEMAN_ENABLED_STERO    
    ...
    if (app_ai_is_in_tws_mode(0))
    {
        if (app_ai_tws_link_connected() && (ai_tws_role != APP_AI_TWS_MASTER))
        {
            AI_VOICE_TRACE(2,"%s role %d isn't MASTER", __func__, ai_tws_role);
            return;
        }
    }
#endif
...
#ifndef FREEMAN_ENABLED_STERO
    if (app_ai_is_in_tws_mode(0) && APP_AI_TWS_MASTER == ai_tws_role)
    {
        app_ai_tws_sync_ai_info();
    }
#endif
...
uint8_t app_ai_connect_handle(AI_TRANS_TYPE_E ai_trans_type, uint8_t ai_index, uint8_t dest_id, uint8_t *addr)
{
    ...
    if (ai_connect_index >= AI_CONNECT_NUM_MAX)
    {
        AI_VOICE_TRACE(0, "AI connected device number has been MAX!!!");
        return ai_connect_index;
    }
...
void app_ai_disconnect_handle(AI_TRANS_TYPE_E ai_trans_type, uint8_t ai_index)
{
    ...
    if(!app_ai_spec_have_other_connection(ai_index))
    {
        ...
        ai_manager_set_spec_connected_status(app_ai_get_ai_spec(ai_index), 0);
    ...
    if (app_ai_is_in_tws_mode(0))
    {
        ...
        {
            if (ai_tws_role == APP_AI_TWS_MASTER)
            {
                app_ai_tws_sync_ai_info();
            }
            else
...
void app_ai_setup_complete_handle(uint8_t ai_index)
{
    ...
    /// enable AI to handle mic data when connected
    app_ai_voice_set_active_flag(app_ai_voice_get_user_from_spec(ai_index), true);
    ...
    if (app_ai_is_in_multi_ai_mode())
    {
        ...
        {
            AI_VOICE_TRACE(2,"%s spec %d", __func__, ai_index);
            return;
        }
    }
#endif
    ...
    ai_manager_set_spec_connected_status((AI_SPEC_TYPE_E)ai_index, 1);
    if (app_ai_is_use_thirdparty(ai_index) && app_ai_is_enabled_wakeword(ai_index))
    {
        #ifdef __THIRDPARTY
        app_ai_if_thirdparty_event_handle(THIRDPARTY_FUNC_KWS, THIRDPARTY_HW_DETECT_START, ai_index);
        #endif
        app_ai_voice_stream_control(true, app_ai_voice_get_user_from_spec(ai_index));
    }
    else
    {
        AI_VOICE_TRACE(1, "AI(%d) not use thirdparty", ai_index);
    }
...
#ifndef AI_THREAD_SIZE
#define AI_THREAD_SIZE  (1024*4)
#endif
osThreadDef(ai_thread, osPriorityAboveNormal, 1, AI_THREAD_SIZE, "ai_thread");
...
static int ai_mailbox_init(void)
{
    ...
    if (ai_mailbox == NULL)
    {
        AI_VOICE_TRACE(0,"Failed to Create ai_mailbox\n");
        return -1;
    }
    ai_mailbox_cnt = 0;
    return 0;
...
int ai_mailbox_put(AI_SIGNAL_E signal, uint32_t len, uint8_t ai_index, uint8_t dest_id)
{
    ...
    {
        AI_VOICE_TRACE(2,"%s warn ai_mailbox_cnt is %d", __func__, ai_mailbox_cnt);
        return osErrorValue;
    }
    ...
    if (osOK != status)
    {
        AI_VOICE_TRACE(2,"%s error status 0x%02x", __func__, status);
        return (int)status;
    }
...
int ai_mailbox_get(AI_MESSAGE_BLOCK** msg_p)
{
    ...
    {
       AI_VOICE_TRACE(2,"%s evt.status 0x%02x", __func__, evt.status);
        return -1;
    }
}
        ...
        if(!ai_mailbox_get(&msg_p))
        {
            ...
            switch(signal){
                ...
                case SIGN_AI_WAKEUP:
                    ...
                    // app_ai_control_mute_a2dp_state(true);
                    if (app_ai_is_local_start_tone(ai_index))
                    {
                        ...
                        if(0)
                        {
                            ...
                            media_PlayAudio_standalone_locally(AUDIO_ID_BT_ALEXA_START, 0);
#elif defined(__TENCENT_VOICE__)
                        else if(ai_index == AI_SPEC_TENCENT)
                            media_PlayAudio_standalone_locally(AUDIO_ID_BT_TENCENT_START, 0);
#endif
                        else
                            media_PlayAudio_standalone_locally(AUDIO_ID_BT_GSOUND_MIC_OPEN, 0);
#endif
                    }
                break;
                ...
                break;
                default:
                    AI_VOICE_TRACE(1,"%s error msg", __func__);
                break;
            }
        }
    }
...
static void _common_ai_init()
{
    ...
    if (!instanceflag)
    {
        ...
        if (ai_mailbox_init())
        {
            AI_VOICE_TRACE(0, "ERROR: AI mailbox init failed");
            return;
        }
        ...
        {
            AI_VOICE_TRACE(0, "ERROR: AI thread init failed");
            return;
        }
#endif
    ...
    {
        AI_VOICE_TRACE(0,"AI common already initiated!");
    }
...
void ai_open_specific_ai(uint32_t ai_spec)
{
    ...
    AI_VOICE_TRACE(2, "%s spec:%d", __func__, ai_spec);
    ...
    if (ai_spec == AI_SPEC_BIXBY)
    {
        app_ai_set_local_start_tone(false, ai_index);
    }
    else
    {
    ...
    if(pInstance)
    {
        ...
        AI_VOICE_TRACE(2, "Register func_table(%p) for AI(%d)", ai_handler_function_table, ai_index);
    }
    else
    {
        AI_VOICE_TRACE(2, "WARNING:no instance found for AI(%d)", ai_index);
    }
}
...
uint32_t ai_save_ctx(uint8_t *buf, uint32_t buf_len)
{
    ...
    for (uint8_t connect_index = 0; connect_index < AI_CONNECT_NUM_MAX; connect_index++)
    {
        ...
        memcpy((buf + offset), &ai_connect_info[connect_index], sizeof(ai_connect_info[connect_index]));
        offset += sizeof(ai_connect_info[connect_index]);
        ...
        *(uint32_t *)(buf + offset) = app_ai_get_data_trans_size(ai_index);
        ...
        AI_VOICE_TRACE(3,"%s offset %d setup %d",
              __func__,
              offset,
...
uint32_t ai_restore_ctx(uint8_t *buf, uint32_t buf_len)
{
    ...
    for (uint8_t connect_index = 0; connect_index < AI_CONNECT_NUM_MAX; connect_index++)
    {
        ...
        offset += sizeof(ai_struct[ai_index].ai_mobile_info);
        memcpy(&ai_connect_info[connect_index], (buf + offset), sizeof(ai_connect_info[connect_index]));
        offset += sizeof(ai_connect_info[connect_index]);
        ...
        AI_VOICE_TRACE(2,"%s peer setup %d", __func__, app_ai_is_peer_setup_complete(ai_index));
