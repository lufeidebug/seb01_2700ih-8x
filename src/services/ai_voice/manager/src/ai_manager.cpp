...
{
    AI_VOICE_TRACE(0,"cross adv timer callback handler");
...
POSSIBLY_UNUSED static void ai_manager_disconnect_all_bt_connection(void)
{
...
bool ai_voicekey_is_enable(void)
{
    ...
    AI_VOICE_TRACE(2,"%s voicekey_en %d", __func__, nvrecord_env->aiManagerInfo.voice_key_enable);
    return (nvrecord_env->aiManagerInfo.voice_key_enable);
}

void ai_voicekey_save_status(bool state)
{
    AI_VOICE_TRACE(2,"%s state: %d", __func__, state);
    struct nvrecord_env_t *nvrecord_env;
    nv_record_env_get(&nvrecord_env);
...
void ai_manager_init(void)
{
    ...
    AI_VOICE_TRACE(5,"%s spec: %d saved: %d ai_disable:%d ama_assistant:%d",
          __func__,
...
void ai_manager_switch_spec(AI_SPEC_TYPE_E ai_spec)
{
    ...
    if (ai_spec == spec_temp)
    {
        AI_VOICE_TRACE(2,"%s spec: %d no change, ignore", __func__, ai_spec);
        return;
    }
    ...
    if(!ai_voicekey_is_enable()){
        AI_VOICE_TRACE(0,"No AI was setup ever!\n");
        return;
    }

    if (AI_SPEC_AMA == spec_temp)
    {
        uint8_t dest_id = app_ai_get_dest_id_by_ai_spec(AI_SPEC_AMA);
...
void ai_manager_set_current_spec(AI_SPEC_TYPE_E ai_spec)
{
    ...
    AI_VOICE_TRACE(3,"%s %s->%s", __func__, ai_spec_type2str((AI_SPEC_TYPE_E)aiManager.info.currentAiSpec), ai_spec_type2str(ai_spec));

    if (aiManager.info.currentAiSpec == ai_spec)
    {
        AI_VOICE_TRACE(0,"ai_spec has set");
        return;
    }
    ...
    if (AI_SPEC_AMA == ai_spec)
    {
        if (AI_SPEC_GSOUND == aiManager.info.currentAiSpec)
        {
            ...
            else
            {
                ...
                ai_manager_set_spec_update_flag(AI_SPEC_GSOUND, true);
            }
        }
    }
    else if (AI_SPEC_GSOUND == ai_spec)
    {
        if (AI_SPEC_AMA == aiManager.info.currentAiSpec)
        {
            ...
            {
                aiManager.reboot_ongoing = true;
                ai_manager_set_spec_update_flag(AI_SPEC_AMA, true);
            }
        }
    }
    ...
    else if(AI_SPEC_INIT == ai_spec)
    {
        ...
        {
            uint8_t dest_id = app_ai_get_dest_id_by_ai_spec(AI_SPEC_TENCENT);
            ai_function_handle(CALLBACK_DISCONNECT_APP, NULL, 0, AI_SPEC_TENCENT, dest_id);
            bes_ble_gap_force_switch_adv(BLE_SWITCH_USER_AI, TRUE);
        }
    }
#endif
...
{
    AI_VOICE_TRACE(2, "%s need_reboot %d", __func__, aiManager.need_reboot);
    return aiManager.need_reboot;
}

void ai_manager_set_spec_connected_status(uint8_t ai_spec, uint8_t connected)
{
    if (ai_spec < 0 || ai_spec >= AI_SPEC_COUNT)
    {
        AI_VOICE_TRACE(1,"%s invaild parameter", __func__);
        return;
    }
    AI_VOICE_TRACE(4,"%s %d%s connected %d", __func__, ai_spec, ai_spec_type2str((AI_SPEC_TYPE_E)ai_spec), connected);
...
int8_t ai_manager_get_spec_connected_status(uint8_t ai_spec)
{
    if (ai_spec < 0 || ai_spec >= AI_SPEC_COUNT)
    {
        AI_VOICE_TRACE(0,"invaild parameter");
        return -1;
    }
...
{
    AI_VOICE_TRACE(1,"ai_manager_set_ama_assistant_status:%d", ama_assistant_value);
    aiManager.rebootAmaEnable = ama_assistant_value;
}

void ai_manager_set_spec_update_flag(uint8_t ai_spec, bool onOff)
{
    if (ai_spec < 0 || ai_spec >= AI_SPEC_COUNT){
        AI_VOICE_TRACE(0,"invaild parameter");
        return;
    }
    AI_VOICE_TRACE(2,"%s ai_spec:%d,onOff:%d", __func__,ai_spec, onOff);
    if(onOff){
        aiManager.specUpdateStart |= (ai_spec);
    }else{
        aiManager.specUpdateStart &= ~(ai_spec);
    }
}
...
{
    //AI_VOICE_TRACE(2,"%s %d", __func__, aiManager.specUpdateStart);
...
void ai_manager_spec_update_start_reboot()
{
    ...
    AI_VOICE_TRACE(4,"ai_manager_reboot saveSpec:%d%s, ai_invalid_flag:%d reboot:%d",
                                      ai_spec,
                                      ai_spec_type2str(ai_spec),
                                      aiManager.disableFlag,
                                      aiManager.reboot_ongoing);
...
void ai_manager_set_foreground_ai_conidx(uint8_t ai_connect_index)
{
    ...
    {
        AI_VOICE_TRACE(2, "%s, can not set foreground ai, index%d has not ai connected", __func__, ai_connect_index);
        return;
    }

    if(ai_connect_index < AI_CONNECT_NUM_MAX)
    {
        AI_VOICE_TRACE(2, "Foreground AI update:%d->%d", ForegroundAI, ai_connect_index);
        ForegroundAI = ai_connect_index;
    }
    else
    {
        AI_VOICE_TRACE(2, "%s, error connection index %d", __func__, ai_connect_index);
...
{
    //add to ai manager
    AI_VOICE_TRACE(3,"%s ai_idx:%d, isAdd:%d", __func__, ai_connect_index, AddorRemove);
    if(AddorRemove) {
    ...
    //remove from ai manager
        if(ai_connect_index < AI_CONNECT_NUM_MAX && (ForegroundAI == ai_connect_index))
        {
            for(uint8_t i = 0; i < AI_CONNECT_NUM_MAX && (i != ForegroundAI); i++){
                ...
                if (AI_IS_CONNECTED == ai_info->ai_connect_state) {
                    ForegroundAI = i;
                    AI_VOICE_TRACE(2, "Remove current AI, Foreground AI update:%d->%d", ai_connect_index, ForegroundAI);
                    return;
                }
            }
            ForegroundAI = AI_CONNECT_INVALID;
...
{
    //AI_VOICE_TRACE(1, "ForegroundAI:%d", ForegroundAI);
    return ForegroundAI;
}
...
uint32_t ai_manager_save_ctx(uint8_t *buf, uint32_t buf_len)
{
    ...
    AI_VOICE_TRACE(2,"%s len %d", __func__, buf_len);
...
uint32_t ai_manager_restore_ctx(uint8_t *buf, uint32_t buf_len)
{
    ...
    if (buf_len == sizeof(aiManager))
    {
        ...
        if (aiManager.reboot_ongoing)
        {
            AI_VOICE_TRACE(1,"%s reboot", __func__);
            osTimerStart(ai_manager_reset_timer_id, 
    ...
    {
        AI_VOICE_TRACE(2,"%s len error %d", __func__, buf_len);
    }
...
{
    AI_VOICE_TRACE(2,"%s reboot %d", __func__, aiManager.reboot_ongoing);
    if (aiManager.reboot_ongoing)
    {
...
void ai_manager_update_gsound_spp_info(uint8_t* _addr, uint8_t device_id, uint16_t connect_state)
{
    uint8_t ai_connect_index = AI_CONNECT_INVALID;
    if (BT_SPP_EVENT_OPENED == connect_state)
    {
        ai_connect_index = app_ai_set_spp_connect_info(_addr, device_id, AI_TRANSPORT_SPP, AI_SPEC_GSOUND);
    }
    else if (BT_SPP_EVENT_CLOSED == connect_state)
    {
    ...
    ai_manager_update_foreground_ai_conidx(ai_connect_index, (connect_state == BT_SPP_EVENT_OPENED) ? true : false);
}
...
{
    uint8_t dest_id = 0;
    uint8_t ai_spec = ai_manager_get_current_spec();
    ...
    {
        AI_VOICE_TRACE(2, "%s isn't master %d", __func__, app_ai_tws_get_local_role());
        app_ibrt_keyboard_notify_v2(NULL, status, param);
        return;
    }
#endif

    AI_VOICE_TRACE(2,"%s event %d", __func__, status->event);

#ifdef MAI_TYPE_REBOOT_WITHOUT_OEM_APP
    if (app_ai_manager_get_spec_update_flag())
    {
        AI_VOICE_TRACE(0,"device reboot is ongoing...");
        return;
    }
#endif

    if (AI_SPEC_GSOUND == ai_spec)
    {
        AI_VOICE_TRACE(1, "%s foreground ai is Gsound", __func__);
#ifdef BISTO_ENABLED
...
#ifdef __AMA_VOICE__
    if (get_alexa_service_enable_state() == 0)
    {
        return;
    }
#endif
    ...
    app_ai_set_wake_up_type(AIV_WAKEUP_TRIGGER_PRESS_AND_HOLD, ai_spec);
#else
    app_ai_set_wake_up_type(AIV_WAKEUP_TRIGGER_TAP, ai_spec);
#endif

    dest_id = app_ai_get_dest_id_by_ai_spec(ai_spec);
    if(0xFF == dest_id) {
        return;
    }
    if (0 == ai_function_handle(CALLBACK_KEY_EVENT_HANDLE, status, (uint32_t)param, ai_spec, dest_id))
    {
        if (app_ai_is_use_thirdparty(ai_spec))
        {
            #ifdef __THIRDPARTY
            app_ai_if_thirdparty_event_handle(THIRDPARTY_FUNC_KWS,THIRDPARTY_AI_PROVIDE_SPEECH, ai_spec);
            #endif
        }
    }
#endif
...
AI_DEVICE_ACTION_TYPE_T ai_manager_switch_to_device_action(AI_SPEC_TYPE_E spec,uint32_t event)
{
    ...
    AI_VOICE_TRACE(2,"%s device_action=%d",__func__, device_action);
...
{
    bool ret = true;
	if(param->event == STATE_WAKE_WORD_ENABLE)
	{
        param->value_type = AI_VALUE_BOOL;
        param->p.booler = app_ai_is_enabled_wakeword(spec);
	}
	else
	{
            ...
            case AI_DEVICE_ACTION_ANC_ENABLE:
                {
                    ...
                    AI_VOICE_TRACE(1,"%s anc enable",__func__);
                }
            break;
            case AI_DEVICE_ACTION_ANC_LEVEL:
                {
                    ...
                    param->p.inter = 0;
                    AI_VOICE_TRACE(2,"%s anc level %d",__func__, param->p.inter);
                }
            break;
            case AI_DEVICE_ACTION_FB_LEVEL:
                {
                    param->value_type = AI_VALUE_INT;
                    param->p.inter = 0;
                    AI_VOICE_TRACE(2,"%s fb anc level %d",__func__, param->p.inter);
                }
            break;
            case AI_DEVICE_ACTION_PASS_THROUGH:
                {
                    param->value_type = AI_VALUE_INT;
                    param->p.inter = 0;
                    AI_VOICE_TRACE(2,"%s pass_through enable %d",__func__, param->p.inter);
                }
            break;
            case AI_DEVICE_ACTION_BATTERY_LEVEL:
                {
                    param->value_type = AI_VALUE_INT;
                    param->p.inter = 0;
                    AI_VOICE_TRACE(2,"%s battery_level %d",__func__, param->p.inter);
                }
            break;
...
bool ai_manager_set_device_status(AI_SPEC_TYPE_E spec, AI_DEVICE_ACTION_PARAM_T *param)
{
	...
	if(param->event == STATE_WAKE_WORD_ENABLE)
	{
        app_ai_enable_wakeword(param->p.booler, spec);
	}
	else
	{
        AI_DEVICE_ACTION_TYPE_T device_action = ai_manager_switch_to_device_action(spec,param->event);
        ...
        switch(device_action)
        {
            case AI_DEVICE_ACTION_ANC_ENABLE:
                if(param->p.booler) {
                    ...
                    AI_VOICE_TRACE(1,"%s anc disable",__func__);
                }
            break;
            case AI_DEVICE_ACTION_ANC_LEVEL:
                AI_VOICE_TRACE(2,"%s anc level %d",__func__, param->p.inter);
            ...
            case AI_DEVICE_ACTION_PASS_THROUGH:
                if(param->p.booler) {
                    AI_VOICE_TRACE(1,"%s pass_through enable",__func__);
                }else {
                    AI_VOICE_TRACE(1,"%s pass_through disable",__func__);
                }
            break;
            case AI_DEVICE_ACTION_BATTERY_LEVEL:
                AI_VOICE_TRACE(2,"%s battery_level %d",__func__, param->p.inter);
