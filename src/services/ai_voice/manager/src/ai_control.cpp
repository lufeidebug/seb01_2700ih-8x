    ...
    if (ai_function_table[ai_index])
    {
        ...
        for (counter = ai_function_table[ai_index]->func_cnt; 0 < counter; counter--)
        {
            ...
            if(handler.id == func_id)
            {
                ...
                {
                    AI_VOICE_TRACE(2,"%s func %d no register", __func__, func_id);
                    break;
                }
...
void app_ai_send_cmd_to_peer(ai_cmd_instance_t *ai_cmd)
{
    ...
    AI_VOICE_TRACE(3,"%s cmd 0x%x len %d", __func__, ai_cmd->cmdcode, length);
#endif
}
...
void app_ai_rev_peer_cmd_hanlder(uint16_t rsp_seq, uint8_t *p_buff, uint16_t length)
{
    ...
    if(ai_cmd.cmdcode >= AI_CMD_GMA_SEND_SECRET && NULL == ai_info) {
        AI_VOICE_TRACE(1, "%s ignore cmd code:%x as ai info is null.", __func__, ai_cmd.cmdcode);
        return;
    }
    if(ai_info)
        dest_id = app_ai_get_dest_id(ai_info);

    AI_VOICE_TRACE(3,"%s cmd 0x%x len %d", __func__, ai_cmd.cmdcode, ai_cmd.param_length);
...
{
    AI_VOICE_TRACE(2, "ai_index:%d, func_table:%p", ai_index, func_table);
