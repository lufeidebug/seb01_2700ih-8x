...
extern "C" uint32_t factory_section_tota_se_user_info_switch_status_get(void);
...
static void app_tota_cmd_handler_remove_waiting_rsp_timeout_supervision(uint16_t entryIndex)
{
    ...
    //TOTA_V2_TRACE(1,"decrease the supervisor timer to %d", tota_cmd_handler_env.timeoutSupervisorCount);
...
void app_tota_cmd_handler_add_waiting_rsp_timeout_supervision(uint16_t entryIndex)
{
    ...
    //TOTA_V2_TRACE(1,"increase the supervisor timer to %d", tota_cmd_handler_env.timeoutSupervisorCount);
...
bool app_tota_cmd_filter_bypass(uint16_t cmd)
{
    TOTA_V2_TRACE(0,"%s cmd = 0x%02x",__func__,cmd);
    bool ret =false;
    if(factory_section_tota_se_user_info_switch_status_get() == 1){
        TOTA_V2_TRACE(0,"factory_section_tota_se_user_info_switch_status_get=1");
        ret = true;
    }

    if(ret)
    {
        if(cmd == OP_TOTA_OTA || cmd == OP_TOTA_STRING ||
            cmd == OP_TOTA_CONN_INITIATE || cmd == OP_TOTA_CONN_RESPONSE || cmd == OP_TOTA_CONN_CONFIRM
        )
        {
            TOTA_V2_TRACE(0,"OP_TOTA_OTA -> filter bypass");
            ret = false;
        }
    }
...
{
    TOTA_V2_TRACE(0,"TOTA Receive data:");
    ...
    {
        TOTA_V2_TRACE(1,"[%s]error TOTA_INVALID_CMD",__func__);
    ...
    {
        TOTA_V2_TRACE(3,"[%s]error TOTA_PARAMETER_LENGTH_TOO_SHORT:0x%x > 0x%x ",__func__,pPayload->paramLen, dataLength);
    ...
    if(app_tota_cmd_filter_bypass((pPayload->cmdCode)) == false)
    {
        if(pInstance) {
