...
static BYTE fatfs_alloc_slot(FAR struct inode *drv)
{
    ...
    if (g_lock == NULL)
        g_lock = fs_mutex_create("fatfs_g_lock");

    fs_mutex_lock(g_lock);

    for (i = 0; i < FF_VOLUMES; i++) {
        if (g_drv[i].drv == NULL) {
            g_drv[i].drv = drv;
            g_drv[i].ratio = 1;
            fs_mutex_unlock(g_lock);
            return i;
        }
    }
...
static int fatfs_convert_result(FRESULT ret)
{
    switch (ret) {
        ...
        case FR_MKFS_ABORTED:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -EIO);
            return -EIO;
        ...
        case FR_INVALID_PARAMETER:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -EINVAL);
            return -EINVAL;

        case FR_DENIED:
        case FR_WRITE_PROTECTED:
        case FR_LOCKED:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -EPERM);
            return -EPERM;

        case FR_EXIST:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -EEXIST);
            return -EEXIST;

        case FR_TIMEOUT:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -ETIMEDOUT);
            return -ETIMEDOUT;

        case FR_NOT_ENOUGH_CORE:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -ENOMEM);
            return -ENOMEM;

        case FR_TOO_MANY_OPEN_FILES:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -ENMFILE);
            return -ENMFILE;

        case FR_INT_ERR:
        default:
            AOS_FS_TRACE(0, FS_ERR_TAG"res: %d ret: %d", __FS_ERR_WHERE__, ret, -EFAULT);
            return -EFAULT;
    }
}
                      ...
                      int oflags, mode_t mode)
{
    ...
    fp = fs_mm_zalloc(sizeof(*fp) - FF_MAX_SS + SS(&fs->fat));
    if (!fp) {
        return -ENOMEM;
    }
...
errsem:
    ...
    AOS_FS_TRACE(2, FS_DEBUG_TAG"ret=%d oflags=%d", __FS_ERR_WHERE__, ret, oflags);
    return ret;
}
...
errout_with_sem:
    fs_mutex_unlock(fs->lock);
    AOS_FS_TRACE(2, FS_DEBUG_TAG"ret=%d", __FS_DEBUG_WHERE__, ret);
    return ret;
}
...
static off_t fatfs_seek(FAR struct file *filep, off_t offset, int whence)
{
    ...
    fs_mutex_unlock(fs->lock);
    AOS_FS_TRACE(2, FS_DEBUG_TAG"offset=%ld whence=%d ret=%d", __FS_DEBUG_WHERE__, offset, whence, (int)(ret < 0 ? ret : offset));
                      ...
                      FAR void **handle)
{
    ...
    if (driver->u.i_bops->open) {
        ...
        if (ret < 0) {
            AOS_FS_TRACE(0, FS_ERR_TAG"%d driver open failed", __FS_ERR_WHERE__, fs->pdrv);
        ...
        sector = fs_mm_zalloc(geo.geo_sectorsize);
        if (sector == NULL) {
            ret = -ENOMEM;
            goto errout_with_open;
        }
        ...
        if (ret < 0) {
            AOS_FS_TRACE(0, FS_ERR_TAG"Read failed: %d", __FS_ERR_WHERE__, ret);
            fs_mm_free(sector);
            goto errout_with_open;
        }
        ...
        if (g_drv[fs->pdrv].ratio != 1) {
            if (g_drv[fs->pdrv].ratio != CONFIG_FS_FATFS_SECTOR_RATIO)
    ...
    AOS_FS_TRACE(0, "\n");
    AOS_FS_TRACE(0, "****fatfs driver geometry****");
    AOS_FS_TRACE(0, "      geo_nsectors : %d", (int)geo.geo_nsectors);
    AOS_FS_TRACE(0, "    geo_sectorsize : %d\n", (int)geo.geo_sectorsize);

    AOS_FS_TRACE(0, "****fatfs ioctl geometry****");
    AOS_FS_TRACE(0, "             RATIO : %d", g_drv[fs->pdrv].ratio);
    AOS_FS_TRACE(0, "  GET_SECTOR_COUNT : %d", (int)geo.geo_nsectors / g_drv[fs->pdrv].ratio);
    AOS_FS_TRACE(0, "   GET_SECTOR_SIZE : %d", (int)geo.geo_sectorsize * g_drv[fs->pdrv].ratio);
    AOS_FS_TRACE(0, "    GET_BLOCK_SIZE : %d\n", (int)geo.geo_sectorsize * g_drv[fs->pdrv].ratio);

    fs->lock = fs_mutex_create("fs_file_lock"); /* Initialize the access control semaphore */
    ...
    if (data && fatfs_match_option(data, "forceformat")) {
        ...
        if (ret < 0) {
            ...
            if (ret < 0) {
                AOS_FS_TRACE(0, FS_ERR_TAG"ret: %d", __FS_ERR_WHERE__, ret);
                goto errout_with_open;
            }
        }
    }
...
DRESULT disk_read(BYTE pdrv, BYTE *buff, LBA_t sector, UINT count)
{
    ...
    if (size != count) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Read failed, size: %d", __FS_ERR_WHERE__, size);
        return RES_ERROR;
    }
...
DRESULT disk_write(BYTE pdrv, const BYTE *buff, LBA_t sector, UINT count)
{
    ...
    if (size != count) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Write failed, size: %d", __FS_ERR_WHERE__, size);
        return RES_ERROR;
    }
    AOS_FS_TRACE(2, FS_DEBUG_TAG"sectorCount:%d", __FS_ERR_WHERE__, size);
    return RES_OK;
}
...
DRESULT disk_ioctl(BYTE pdrv, BYTE cmd, void *buff)
{
    ...
    switch (cmd) {
        ...
        case GET_BLOCK_SIZE: {
            ...
            if (ret < 0) {
                AOS_FS_TRACE(0, FS_ERR_TAG"geometry error, ret: %d", __FS_ERR_WHERE__, ret);
                break;
            }
