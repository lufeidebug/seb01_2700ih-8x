                        ...
                        if (newnode != node) {
                            ...
                            if (INODE_IS_MOUNTPT(newnode)) {
                                ...
                                if (*desc->relpath != '\0') {
                                    char *buffer = NULL;
                                    int len;
                                    len = strlen(desc->relpath) + strlen(name) + 2;
                                    buffer = fs_mm_zalloc(len);
                                    if (buffer == NULL) {
                                        ret = -ENOMEM;
                                    } else {
                                        sprintf(buffer,
...
int inode_search(FAR struct inode_search_s *desc)
{
    ...
    if (*desc->path != '/') {
        len = strlen(_inode_getcwd()) + strlen(desc->path) + 2;
        desc->buffer = fs_mm_zalloc(len);
        if (desc->buffer == NULL) {
            return -ENOMEM;
        }
        sprintf(desc->buffer, "%s/%s", _inode_getcwd(), desc->path);
