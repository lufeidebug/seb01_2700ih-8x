...
static FAR struct inode *inode_alloc(FAR const char *name, mode_t mode)
{
    ...
    node    = (FAR struct inode *)fs_mm_zalloc(FSNODE_SIZE(namelen));
    if (node) {
        node->i_ino   = g_ino++;
...
void inode_root_reserve(void)
{
    ...
    inode_alloc("/dev", 0777);
    inode_alloc("/data", 0777);
    inode_alloc("/dev/partition", 0777);
    inode_alloc("/data/partition", 0777);
}
