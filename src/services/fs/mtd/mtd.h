...
#ifndef __INCLUDE_AOS_MTD_MTD_H
#define __INCLUDE_AOS_MTD_MTD_H
...
#define MTDIOC_FACTOR     _MTDIOC(0x000c) /* IN:  Pointer to uint8_t
                                           * OUT: Byte value that represents the
                                           *      erased state of the MTD cell */
...
struct mtd_factor_conf {
    uint32_t program_size_factor;     /* CONFIG_FS_LITTLEFS_PROGRAM_SIZE_FACTOR */
    uint32_t read_size_factor;        /* CONFIG_FS_LITTLEFS_PROGRAM_SIZE_FACTOR */
    uint32_t block_size_factor;       /* CONFIG_FS_LITTLEFS_BLOCK_SIZE_FACTOR */
...
/****************************************************************************
 * Name: bes_ram_slotinitialize
 *
 * Description:
 *   Initialize one slot for operation using the ram addr
 *
 * Input Parameters:
 *   minor - The ram minor device number.  The ram addr will be
 *     registered as /ramN where N is the minor number
 *
 ****************************************************************************/
int bes_ram_slotinitialize(int minor);
 ...
 *     registered as /dev/nandblockN where N is the minor number
 *   flashid - nand flashid
 *
 ****************************************************************************/
int bes_nand_slotinitialize_dhara(int minor, int flashid);

/****************************************************************************
 * Name: bes_nand_slotinitialize_partition_dhara
 *
 * Description:
 *   Initialize one slot for operation using the nand interface
 *
 * Input Parameters:
 *   minor - The nand minor device number.  The nand device will be
 *     registered as /dev/nandblockN_M where N is the minor number
                                         and M is partition number
 *   flashid - nand flashid
 *   partidx - partition indx
 *
 ****************************************************************************/
 ...
 *   start - partition start address, such as 0x600000/0x28600000/0x2C600000/0x2A600000/0x2D600000
 ...
 *
 * Description:
 *   Initialize bes_nand_slotinitialize_partition or bes_nand_slotinitialize
 *
 ****************************************************************************/
void bes_nand_drivers_register(void);

/****************************************************************************
 * Name: bes_norflash_drivers_register
 *
 * Description:
