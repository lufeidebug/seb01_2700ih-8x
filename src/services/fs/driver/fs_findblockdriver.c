                     ...
                     FAR struct inode **ppinode)
{
    ...
    ret = inode_find(&desc);
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"ERROR: Failed to find %s", __FS_ERR_WHERE__, pathname);
        ret = -ENOENT;
    ...
    if (!INODE_IS_BLOCK(inode)) {
        ...
        if (INODE_IS_MTD(inode)) {
            AOS_FS_TRACE(0, "%s is a MTD", pathname);
        } else
#endif
        {
            AOS_FS_TRACE(0, FS_ERR_TAG"ERROR: %s is not a block driver", __FS_ERR_WHERE__, pathname);
        }
    ...
    if (!inode->u.i_bops || !inode->u.i_bops->read ||
        ...
        AOS_FS_TRACE(0, FS_ERR_TAG"ERROR: %s does not support requested access", __FS_ERR_WHERE__, pathname);
        ret = -EACCES;
        goto errout_with_inode;
    }
