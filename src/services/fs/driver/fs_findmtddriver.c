...
int find_mtddriver(FAR const char *pathname, FAR struct inode **ppinode)
{
    ...
    ret = inode_find(&desc);
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"ERROR: Failed to find, path: %s", __FS_ERR_WHERE__, pathname);
        ret = -ENOENT;
    ...
    if (!INODE_IS_MTD(inode)) {
        AOS_FS_TRACE(0, FS_ERR_TAG"ERROR: %s is not a named MTD driver", __FS_ERR_WHERE__, pathname);
        ret = -ENOTBLK;
