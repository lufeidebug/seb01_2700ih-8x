                     ...
                     FAR struct inode **ppinode)
{
...
#else
        AOS_FS_TRACE(0, FS_ERR_TAG"ERROR: Failed to file %s block driver", __FS_ERR_WHERE__, pathname);
        return ret;
#endif
    }
    ...
    if (inode->u.i_bops->open) {
        ...
        if (ret < 0) {
            AOS_FS_TRACE(0, FS_ERR_TAG"ERROR: %s driver open failed", __FS_ERR_WHERE__, pathname);
            inode_release(inode);
            return ret;
        }
    }
