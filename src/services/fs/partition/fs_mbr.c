                        ...
                        FAR void *arg)
{
    ...
    buffer = fs_mm_zalloc(num * state->blocksize);
    if (!buffer) {
        return -ENOMEM;
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (pentry.nblocks != 0) {
            ...
            AOS_FS_TRACE(0, FS_DEBUG_TAG"Skipping empty partition %d", __FS_DEBUG_WHERE__, i);
        }
    }
    ...
    if (extended) {
        ...
        while (1) {
            ...
            if (buffer[0x1fe] != 0x55 || buffer[0x1ff] != 0xaa) {
                AOS_FS_TRACE(0, FS_ERR_TAG"block %x doesn't contain an EBR signature",
                             __FS_ERR_WHERE__, ebr_block);
                ret = -EINVAL;
                goto out;
            }
            ...
            for (i = 0x1de; i < 0x1fe; ++i) {
                if (buffer[i]) {
                    AOS_FS_TRACE(0, FS_ERR_TAG"EBR's third or fourth partition non-empty",
                                 __FS_ERR_WHERE__);
                    ret = -EINVAL;
                    goto out;
                }
