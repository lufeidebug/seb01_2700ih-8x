...
static FAR struct gpt_entry_s *gpt_alloc_verify_entries(FAR struct partition_state_s *state,
        FAR struct gpt_header_s *gpt)
{
    ...
    pte = fs_mm_zalloc(blk * state->blocksize);
    if (!pte) {
        return NULL;
    }
    ...
    if (ret < 0) {
        fs_mm_free(pte);
        AOS_FS_TRACE(0, FS_ERR_TAG"Read ptr from block failed:%d.", __FS_ERR_WHERE__, ret);
        return NULL;
    }

    /* Check the GUID Partition Table Entry Array CRC */

    crc = crc32_c(~0l, (FAR const uint8_t *)pte, size) ^ ~0l;
    if (crc != le32toh(gpt->partition_entry_array_crc32)) {
        AOS_FS_TRACE(0, FS_ERR_TAG"GUID Partitition Entry Array CRC check failed.", __FS_ERR_WHERE__);
        fs_mm_free(pte);
        return NULL;
    }
                               ...
                               FAR struct gpt_header_s *gpt, blkcnt_t lba)
{
    ...
    if (le64toh(gpt->signature) != GPT_HEADER_SIGNATURE) {
        AOS_FS_TRACE(0, FS_ERR_TAG"GUID Partition Table Header signature is wrong:"
                     "0x%" PRIx64 " != 0x%llx",
                     __FS_ERR_WHERE__,
    ...
    gpt->header_crc32 = 0;
    crc = crc32_c(~0l, (FAR const uint8_t *)gpt,
                  le32toh(gpt->header_size)) ^ ~0l;
    if (crc != le32toh(origcrc)) {
        AOS_FS_TRACE(0, FS_ERR_TAG"GUID Partition Table Header CRC is wrong: %" PRIx32
                     " != %" PRIx32 "\n", __FS_ERR_WHERE__, crc, le32toh(origcrc));
        return -EINVAL;
    }
    ...
    if (le64toh(gpt->my_lba) != lba) {
        AOS_FS_TRACE(0, FS_ERR_TAG"GPT: my_lba incorrect", __FS_ERR_WHERE__);
        return -EINVAL;
    }
    ...
    if (le64toh(gpt->first_usable_lba) > lastlba) {
        AOS_FS_TRACE(0, FS_ERR_TAG"GPT: first_usable_lba incorrect", __FS_ERR_WHERE__);
        return -EINVAL;
    }

    if (le64toh(gpt->last_usable_lba) > lastlba) {
        AOS_FS_TRACE(0, FS_ERR_TAG"GPT: last_usable_lba incorrect", __FS_ERR_WHERE__);
        return -EINVAL;
    }
                        ...
                        FAR void *arg)
{
    ...
    count = (sizeof(struct gpt_ptable_s) + (state->blocksize - 1)) /
            state->blocksize;
    ptbl = fs_mm_zalloc(count * state->blocksize);
    if (!ptbl) {
        return -ENOMEM;
    }
    ...
    if (ret < 0 || !ptes) {
        ...
        AOS_FS_TRACE(0, FS_ERR_TAG"Primary GPT is invalid, using alternate GPT.", __FS_ERR_WHERE__);
    ...
    if (ret < 0 || !ptes) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Alternate GPT is also invalid!!", __FS_ERR_WHERE__);
        goto err;
    }
