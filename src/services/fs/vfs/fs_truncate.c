...
int file_truncate(FAR struct file *filep, off_t length)
{
    ...
    if ((filep->f_oflags & O_WROK) == 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"WARNING: Cannot truncate a file opened read-only", __FS_ERR_WHERE__);
        return -EINVAL;
    }
    ...
    if (inode->u.i_ops == NULL) {
        AOS_FS_TRACE(0, FS_ERR_TAG"WARNING:  Not a file", __FS_ERR_WHERE__);
        return -EINVAL;
    }
    ...
    if (inode->u.i_ops->write == NULL) {
        AOS_FS_TRACE(0, FS_ERR_TAG"WARNING: File system is read-only", __FS_ERR_WHERE__);
        return -EROFS;
    }
    ...
    if (inode->u.i_ops->truncate == NULL) {
        AOS_FS_TRACE(0, FS_ERR_TAG"WARNING: File system does not support the truncate() method", __FS_ERR_WHERE__);
        return -ENOSYS;
    }
...
int ftruncate(int fd, off_t length)
{
    ...
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"FS_ERROR: Could no get file structure: %d", __FS_ERR_WHERE__, ret);
        goto errout;
    }
    ...
    AOS_FS_TRACE(0, FS_ERR_TAG"WARNING: file_truncate() failed: %d", __FS_ERR_WHERE__, ret);
