 ...
 *
 * Description:
 *   fs_basename() extracts the filename component from a null-terminated
...
static FAR char *fs_basename(FAR char *path)
{
    FAR char *p;
    size_t    len;
    ...
    if (ret >= 0) {
        ...
        if (newinode->u.i_ops == NULL || newinode->i_child != NULL) {
            ...
            subdirname = fs_basename((FAR char *)oldpath)
                         len = strlen(newpath) + strlen(subdirname) + 2;
            subdir = fs_mm_zalloc(len);
            if (subdir == NULL) {
                ret = -ENOMEM;
                goto errout;
            }
...
next_subdir:
        ...
        if (ret >= 0) {
            ...
            if (S_ISDIR(buf.st_mode)) {
                ...
                subdirname = fs_basename((FAR char *)oldrelpath);
                ...
                if (*newrelpath == '\0') {
                    ...
                    len = strlen(newrelpath) + strlen(subdirname) + 2;
                    subdir = fs_mm_zalloc(len);
                    if (subdir == NULL) {
                        ret = -ENOMEM;
                        goto errout_with_newinode;
                    }
                    sprintf(subdir, "%s/%s", newrelpath, subdirname);
                    newrelpath = subdir;
                }
