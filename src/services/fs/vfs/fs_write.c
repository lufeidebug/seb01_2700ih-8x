                   ...
                   size_t nbytes)
{
    ...
    if ((filep->f_oflags & O_WROK) == 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG" oflags=%d no access", __FS_ERR_WHERE__, filep->f_oflags);
        return -EACCES;
    }
#endif
    ...
    if (!inode || !inode->u.i_ops || !inode->u.i_ops->write) {
        AOS_FS_TRACE(0, FS_ERR_TAG"bad file", __FS_ERR_WHERE__);
        return -EBADF;
    }
 ...
 *
 * Description:
 *  bes_aos_write() writes up to nytes bytes to the file referenced by the file
...
{
    ssize_t ret;
    /* Let bes_aos_write() do all of the work */
    ret = bes_aos_write(fd, buf, nbytes);
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"fd=%d nbytes=%d ret=%d", __FS_ERR_WHERE__, fd, nbytes, ret);
        set_errno(-ret);
        ret = FS_ERROR;
        return ret;
    }
    AOS_FS_TRACE(2, FS_DEBUG_TAG"fd=%d nbytes=%d ret=%d", __FS_DEBUG_WHERE__, fd, nbytes, ret);
    return ret;
}
#endif

#if (BES_AOS_FS_LIBC_ADAPTER == 1)
FUNC_ALIAS(bes_aos_write, _write, (int fd, const void *buff, size_t bytes), ssize_t);
#endif
