                      ...
                      size_t nblocks)
{
    ...
    if (!part_blockcheck(priv, (startblock + nblocks - 1) * priv->blkpererase)) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Erase beyond the end of the partition", __FS_ERR_WHERE__);
        return -ENXIO;
    }
                          ...
                          size_t nblocks, FAR uint8_t *buf)
{
    ...
    if (!part_blockcheck(priv, startblock + nblocks - 1)) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Read beyond the end of the partition", __FS_ERR_WHERE__);
        return -ENXIO;
    }
                           ...
                           size_t nblocks, FAR const uint8_t *buf)
{
    ...
    if (!part_blockcheck(priv, startblock + nblocks - 1)) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Write beyond the end of the partition", __FS_ERR_WHERE__);
        return -ENXIO;
    }
                            ...
                            int oflags, mode_t mode)
{
    ...
    AOS_FS_TRACE(1, FS_DEBUG_TAG"path=[%s]", __FS_DEBUG_WHERE__, realpath);
    ...
    if ((oflags & O_WRONLY) != 0 || (oflags & O_RDONLY) == 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Only O_RDONLY supported", __FS_ERR_WHERE__);
        return -EACCES;
    }
#endif
    ...
    attr = (FAR struct part_procfs_file_s *)
           fs_mm_zalloc(sizeof(struct part_procfs_file_s));
    if (!attr) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Failed to allocate file attributes", __FS_ERR_WHERE__);
        return -ENOMEM;
    }
                                ...
                                size_t buflen)
{
    ...
    AOS_FS_TRACE(1, FS_DEBUG_TAG"buffer=%p buflen=%d", __FS_DEBUG_WHERE__, buffer, (int)buflen);
    ...
    if (attr->nextpart) {
        ...
        do {
            ...
            if (ret < 0) {
                AOS_FS_TRACE(0, FS_ERR_TAG"mtd->ioctl failed: %zd", __FS_ERR_WHERE__, ret);
                return 0;
            }
                           ...
                           FAR struct file *newp)
{
    ...
    AOS_FS_TRACE(1, FS_DEBUG_TAG"Dup %p->%p", __FS_DEBUG_WHERE__, oldp, newp);
                                    ...
                                    off_t nblocks)
{
    ...
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"mtd->ioctl failed: %d", __FS_ERR_WHERE__, ret);
        return NULL;
    }
    ...
    if (erasestart >= eraseend) {
        AOS_FS_TRACE(0, FS_ERR_TAG"sub-region too small", __FS_ERR_WHERE__);
        return NULL;
    }
    ...
    if (eraseend > geo.neraseblocks) {
        AOS_FS_TRACE(0, FS_ERR_TAG"sub-region too big", __FS_ERR_WHERE__);
        return NULL;
    }
    ...
    part = (FAR struct mtd_partition_s *)
           fs_mm_zalloc(sizeof(struct mtd_partition_s));
    if (!part) {
        AOS_FS_TRACE(0, FS_ERR_TAG"Failed to allocate memory for the partition device", __FS_ERR_WHERE__);
        return NULL;
    }
