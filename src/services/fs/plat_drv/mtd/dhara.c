...
static int dhara_init_readcache(FAR dhara_dev_t *dev)
{
    ...
    do {
        ...
        dev->readpage[i].buffer = fs_mm_zalloc(dev->geo.blocksize);
...
static void dhara_deinit_readcache(FAR dhara_dev_t *dev)
{
    ...
    for (i = 0; i < CONFIG_DHARA_READ_NCACHES; i++) {
        if (dev->readpage[i].buffer) {
            fs_mm_free(dev->readpage[i].buffer);
        }
    }
}
...
static int dhara_open(FAR struct inode *inode)
{
    ...
    fs_mutex_lock(dev->lock);
    dev->refs++;
    fs_mutex_unlock(dev->lock);
...
static int dhara_close(FAR struct inode *inode)
{
    ...
    if (dev->refs == 0 && dev->unlinked) {
        fs_mutex_destroy(&dev->lock);
        dhara_deinit_readcache(dev);
        fs_mm_free(dev->pagebuf);
        fs_mm_free(dev);
    }
                          ...
                          unsigned int nsectors)
{
    ...
    while (nsectors-- > 0) {
        ...
        if (ret < 0) {
            ...
            AOS_FS_TRACE(0, FS_ERR_TAG"Read startblock %lld failed nread %zd err: %s",
                         __FS_DEBUG_WHERE__,
                           ...
                           unsigned int nsectors)
{
    ...
    while (nsectors-- > 0) {
        ...
        if (ret < 0) {
            ret = dhara_convert_result(err);
            AOS_FS_TRACE(0, FS_ERR_TAG"Write starting at block %lld failed nwrite %zu err %s",
                         __FS_DEBUG_WHERE__,
                       ...
                       unsigned long arg)
{
     ...
     */
    if (cmd == BIOC_FLUSH) {
        AOS_FS_TRACE(1, FS_DEBUG_TAG"dhara_map_sync begin", __FS_DEBUG_WHERE__);
        dhara_map_sync(&dev->map, &err);
        AOS_FS_TRACE(1, FS_DEBUG_TAG"dhara_map_sync end, err=%s", __FS_DEBUG_WHERE__, dhara_strerror(err));
        return 0;
    }  else if (cmd == MTDIOC_GEOMETRY) {
        struct mtd_geometry_s *geo = (struct mtd_geometry_s *)((uintptr_t)arg);
        if (geo) {
            memset(geo, 0, sizeof(*geo));
            geo->blocksize    = 1 << dev->nand.log2_page_size;
            geo->erasesize    = 1 << dev->nand.log2_page_size;
            geo->neraseblocks = (1 << dev->nand.log2_ppb) * dev->nand.num_blocks;
            ret               = FS_OK;
            return ret;
        }
        return -ENOTTY;
    ...
    if (ret < 0 && ret != -ENOTTY) {
        AOS_FS_TRACE(0, FS_ERR_TAG"MTD ioctl(%04x) failed: %d", __FS_ERR_WHERE__, cmd, ret);
    }
                    ...
                    FAR dhara_error_t *err)
{
    ...
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"src_page %d, ret %d failed: %s",
                     __FS_ERR_WHERE__, src, ret, dhara_strerror(*err));
        return ret;
    }

    ret = dhara_nand_prog(n, dst, buf, err);
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"dst_page %d, ret %d failed: %s",
                     __FS_ERR_WHERE__, dst, ret, dhara_strerror(*err));
        return ret;
    }
                             ...
                             FAR struct mtd_dev_s *mtd)
{
    ...
    dev = (FAR dhara_dev_t *)
          fs_mm_zalloc(sizeof(dhara_dev_t));
    if (dev == NULL) {
        return -ENOMEM;
    }

    dev->lock = fs_mutex_create("dhara");
    ...
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"MTD ioctl(MTDIOC_GEOMETRY) failed: %d", __FS_ERR_WHERE__, ret);
        goto err;
    }
    ...
    dev->nand.log2_page_size = fs_nand_log2(dev->geo.blocksize);
    dev->nand.log2_ppb = fs_nand_log2(dev->blkper);
    dev->nand.num_blocks = dev->geo.neraseblocks;

    dev->pagebuf = fs_mm_zalloc(dev->geo.blocksize * 2);
    if (!dev->pagebuf) {
        ret = -ENOMEM;
        goto err;
    }
    ...
    if (ret < 0) {
        AOS_FS_TRACE(0, FS_ERR_TAG"register_blockdriver failed: %d", __FS_ERR_WHERE__, ret);
        goto err;
    }
...
int dhara_initialize(int minor, FAR struct mtd_dev_s *mtd)
{
    ...
    snprintf(path, sizeof(path), "/dev/nandblock%d", minor);
