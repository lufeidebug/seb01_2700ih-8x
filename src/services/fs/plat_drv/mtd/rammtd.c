...
static void *ram_write(FAR void *dest, FAR const void *src, size_t len)
{
    ...
    while (len-- > 0) {
        ...
        if (newvalue != srcvalue) {
            AOS_FS_TRACE(0, FS_ERR_TAG"Bad write: source=%02x dest=%02x result=%02x\n",
                         __FS_ERR_WHERE__, srcvalue, oldvalue, newvalue);
        }
#endif
...
FAR struct mtd_dev_s *rammtd_initialize(FAR uint8_t *start, size_t size)
{
    ...
    priv = (FAR struct ram_dev_s *)fs_mm_zalloc(sizeof(struct ram_dev_s));
    if (!priv) {
...
int bes_ram_slotinitialize(int minor)
{
    ...
    uint8_t *buff = NULL;
    uint32_t len = 40 * 1024;
    char devname[16];

    buff = fs_mm_zalloc(len);
    if (buff == NULL) {
        AOS_FS_TRACE(0, FS_ERR_TAG"no memory.", __FS_ERR_WHERE__);
        return -ENOMEM;
    }
    mtd = rammtd_initialize(buff, len);
    if (mtd == NULL) {
        return -1;
    }
