 ...
 ****************************************************************************/
static uint32_t g_fs_init = 0;
uint32_t g_fs_trmask = 0; //(1 << 0)|(1 << 1) | (1 << 2);
...
extern const struct mountpt_operations g_bes_nofs_operations;
...
#endif
#if defined(CONFIG_FS_RPMSGFS) && defined(BES_AOS_RPMSGFS_CLIENT)
extern const struct mountpt_operations g_rpmsgfs_ops;
#endif
             ...
             FAR const void *data)
{
...
#endif /* BDFS_SUPPORT */
        if (mops == NULL) {
            AOS_FS_TRACE(0, FS_ERR_TAG"Failed to find block based file system %s", __FS_ERR_WHERE__, filesystemtype);
            ret = -ENODEV;
...
#endif /* MDFS_SUPPORT */
        if (mops == NULL) {
            AOS_FS_TRACE(0, FS_ERR_TAG"Failed to find MTD based file system %s", __FS_ERR_WHERE__, filesystemtype);
            ret = -ENODEV;
            goto errout_with_inode;
        }
    } else
...
#endif /* NODFS_SUPPORT */
        {
            AOS_FS_TRACE(0, FS_ERR_TAG"Failed to find block driver %s", __FS_ERR_WHERE__, source);
    ...
    if (ret >= 0) {
        ...
        if (!INODE_IS_PSEUDODIR(mountpt_inode)) {
            AOS_FS_TRACE(0, FS_ERR_TAG"target %s exists and is a special node", __FS_ERR_WHERE__, target);
            ret = -ENOTDIR;
...

    {
        ...
        if (ret < 0) {
            ...
            AOS_FS_TRACE(0, FS_ERR_TAG"Failed to reserve inode for target %s", __FS_ERR_WHERE__, target);
            goto errout_with_lock;
        }
    }
    ...
    if (mops->bind == NULL) {
        ...
        AOS_FS_TRACE(0, FS_ERR_TAG"Filesystem does not support bind", __FS_ERR_WHERE__);
        ret = -EINVAL;
    ...
    if (ret < 0) {
        ...
        AOS_FS_TRACE(0, FS_ERR_TAG"Bind method failed: %d", __FS_ERR_WHERE__, ret);
...
#else
    AOS_FS_TRACE(0, FS_ERR_TAG"No filesystems enabled", __FS_ERR_WHERE__);
    return -ENOSYS;
          ...
          FAR const void *data)
{
    ...
    ret = bes_aos_mount(source, target, filesystemtype, mountflags, data);
    if (ret < 0) {
        set_errno(-ret);
        ret = FS_ERROR;
        return ret;
    }
    AOS_FS_TRACE(1, FS_DEBUG_TAG"source dev: %s target path: %s", __FS_DEBUG_WHERE__, source, target);
    return ret;
}
