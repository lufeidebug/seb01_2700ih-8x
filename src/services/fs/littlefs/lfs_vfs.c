                         ...
                         FAR void **handle)
{
    ...
    uint32_t littlefs_factor_conf = 0;
    if (INODE_IS_MTD(driver)) {
        /* Get MTD geometry directly */
        ret = MTD_IOCTL(driver->u.i_mtd, MTDIOC_FACTOR,
                        (unsigned long)&fs->factor);
        if( ret == 0 ) {
            littlefs_factor_conf = 1;
            fs->cfg.context        = fs;
            ...
            fs->cfg.read_size      = fs->geo.blocksize * fs->factor.read_size_factor;
            fs->cfg.prog_size      = fs->geo.blocksize * fs->factor.program_size_factor;
            fs->cfg.block_size     = fs->geo.erasesize * fs->factor.block_size_factor;
            fs->cfg.block_count    = fs->geo.neraseblocks / fs->factor.block_size_factor;
            fs->cfg.block_cycles   = CONFIG_FS_LITTLEFS_BLOCK_CYCLE;
            fs->cfg.cache_size     = fs->geo.blocksize * fs->factor.cache_size_factor;
            ...
            AOS_FS_TRACE(0, "\n");
            AOS_FS_TRACE(0, "****littlefs driver geometry****");
            AOS_FS_TRACE(0, "    geo.neraseblocks: %d", (int)fs->geo.neraseblocks);
            AOS_FS_TRACE(0, "       geo.blocksize: %d", (int)fs->geo.blocksize);
            AOS_FS_TRACE(0, "       geo.erasesize: %d\n", (int)fs->geo.erasesize);

            AOS_FS_TRACE(0, "****littlefs ioctl geometry****");
            AOS_FS_TRACE(0, "          read_size : %d", fs->cfg.read_size);
            AOS_FS_TRACE(0, "          prog_size : %d", fs->cfg.prog_size);
            AOS_FS_TRACE(0, "        block_count : %d", fs->cfg.block_count);
            AOS_FS_TRACE(0, "         block_size : %d", fs->cfg.block_size);
            AOS_FS_TRACE(0, "       block_cycles : %d", fs->cfg.block_cycles);
            AOS_FS_TRACE(0, "         cache_size : %d", fs->cfg.cache_size);
            AOS_FS_TRACE(0, "   READ_SIZE_FACTOR : %d", fs->factor.read_size_factor);
            AOS_FS_TRACE(0, "PROGRAM_SIZE_FACTOR : %d", fs->factor.program_size_factor);
            AOS_FS_TRACE(0, "  BLOCK_SIZE_FACTOR : %d", fs->factor.block_size_factor);
            AOS_FS_TRACE(0, "        BLOCK_CYCLE : %d", CONFIG_FS_LITTLEFS_BLOCK_CYCLE);
            AOS_FS_TRACE(0, "  CACHE_SIZE_FACTOR : %d\n", fs->factor.cache_size_factor);
        }
    }

    if (littlefs_factor_conf == 0) {
        fs->cfg.context        = fs;
        fs->cfg.read           = littlefs_read_block;
        fs->cfg.prog           = littlefs_write_block;
        fs->cfg.erase          = littlefs_erase_block;
        ...
        fs->cfg.block_count    = fs->geo.neraseblocks / CONFIG_FS_LITTLEFS_BLOCK_SIZE_FACTOR;
        fs->cfg.block_cycles   = CONFIG_FS_LITTLEFS_BLOCK_CYCLE;
        ...
        AOS_FS_TRACE(0, "       block_cycles : %d", fs->cfg.block_cycles);
        AOS_FS_TRACE(0, "         cache_size : %d", fs->cfg.cache_size);
        AOS_FS_TRACE(0, "   READ_SIZE_FACTOR : %d", CONFIG_FS_LITTLEFS_READ_SIZE_FACTOR);
        AOS_FS_TRACE(0, "PROGRAM_SIZE_FACTOR : %d", CONFIG_FS_LITTLEFS_PROGRAM_SIZE_FACTOR);
        AOS_FS_TRACE(0, "  BLOCK_SIZE_FACTOR : %d", CONFIG_FS_LITTLEFS_BLOCK_SIZE_FACTOR);
        AOS_FS_TRACE(0, "        BLOCK_CYCLE : %d", CONFIG_FS_LITTLEFS_BLOCK_CYCLE);
        AOS_FS_TRACE(0, "  CACHE_SIZE_FACTOR : %d\n", CONFIG_FS_LITTLEFS_CACHE_SIZE_FACTOR);
    }
