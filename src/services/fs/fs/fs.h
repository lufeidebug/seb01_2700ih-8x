...
#ifndef __INCLUDE_AOS_FS_FS_H
#define __INCLUDE_AOS_FS_FS_H
...
enum bes_nofs_write_flag {
    BES_NOFS_PROC_INIT = 0,
    BES_NOFS_PROC_BLOCK = 1,
    BES_NOFS_PROC_STREAM = 2
};

struct bes_nofs_file_s {
    uint32_t refs;
    uint32_t block;
    uint32_t oflags;
    uint32_t offset;
    uint32_t procflag; //1: block write  2: byte write
    char*    data;
};
...
/* This structure represents one inode in the Aos pseudo-file system */
...
struct filelist {
    void             *fl_lock;    /* Manage access to the file list */
 ...
 * Note that Aos can support the standard C APIs with or without buffering
 *
...
struct streamlist {
    void                   *sl_lock;   /* For thread safety */
 ...
 *
 * Description:
 *   bes_aos_mount() is similar to the standard 'mount' interface except that is
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   bes_aos_mount() is an internal Aos interface and should not be called from
 *   applications.
 *
 * Returned Value:
...
#ifndef CONFIG_DISABLE_MOUNTPOINT
int bes_aos_mount(FAR const char *source, FAR const char *target,
             FAR const char *filesystemtype, unsigned long mountflags,
             FAR const void *data);
#endif

/****************************************************************************
 * Name: bes_aos_umount2
 *
 * Description:
 *   bes_aos_umount2() is similar to the standard 'umount2' interface except that
 *   is not a cancellation point and it does not modify the errno variable.
 *
 *   bes_aos_umount2() is an internal Aos interface and should not be called
 *   from applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
...
#ifndef CONFIG_DISABLE_MOUNTPOINT
int bes_aos_umount2(FAR const char *target, unsigned int flags);
#endif
 ...
 *
 * Description:
 *   bes_aos_open() is similar to the standard 'open' interface except that is is
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   bes_aos_open() is an internal Aos interface and should not be called
 *   from applications.
 *
 * Returned Value:
 *   The new file descriptor is returned on success; a negated errno value is
 ...
 *
 * Description:
 *  bes_aos_write() writes up to nbytes bytes to the file referenced by the file
 *  descriptor fd from the buffer starting at buf.  bes_aos_write() is an
...
ssize_t bes_aos_write(int fd, FAR const void *buf, size_t nbytes);
 ...
 *
 * Description:
 *   bes_aos_fstat() is similar to the standard 'fstat' interface except that is
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   bes_aos_fstat() is an internal Aos interface and should not be called from
 *   applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
...
int bes_aos_fstat(int fd, FAR struct stat *buf);

/****************************************************************************
 * Name: bes_aos_stat
 *
 * Description:
 *   bes_aos_stat() is similar to the standard 'stat' interface except that is
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   bes_aos_stat() is an internal Aos interface and should not be called from
 *   applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
...
int bes_aos_stat(FAR const char *path, FAR struct stat *buf, int resolve);
 ...
 *
 * Description:
 *   bes_aos_unlink() is similar to the standard 'unlink' interface except that
 *   is not a cancellation point and it does not modify the errno variable.
 *
 *   bes_aos_unlink() is an internal Aos interface and should not be called
 *   from applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
...
int bes_aos_unlink(FAR const char *pathname);
...
/****************************************************************************
 * Name: aosfs_initialize
 ****************************************************************************/
void    aosfs_initialize(void);

char *bes_aos_fs_basename(char *path);
#undef EXTERN
#if defined(__cplusplus)
}
#endif

#endif /* __INCLUDE_AOS_FS_FS_H */
