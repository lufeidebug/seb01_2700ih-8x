...
{
    VOICEPATH_TRACE(0,"GSound BT Transmit Callback: channel=%s, len=%d",
    ...
    *bytes_consumed = 0;
    ...
    {
        VOICEPATH_TRACE(0,"slave is not allowed to send data.");
    ...
    {
        VOICEPATH_TRACE(0,"%s not connected", gsound_chnl2str(channel));
                                      ...
                                      const GSoundBTAddr *gsound_addr)
{
    ...
    {
        VOICEPATH_TRACE(0,"gsound target map client send sms: acl not connected %02x:%02x:...:%02x",
...
GSoundStatus GSoundTargetBtPauseSecondary(const GSoundBTAddr *gsound_addr)
{
   ...
   */
    if (GSoundTargetBtNumDevicesConnected() > 1)
    {
        ...
        VOICEPATH_TRACE(0,"GSound: Multi-point detected: %d", GSoundTargetBtNumDevicesConnected());
...
bool GSoundTargetBtIsConnected(GSoundBTAddr const *gsound_addr)
{
    ...
    VOICEPATH_TRACE(0,"converted add:");
    VOICEPATH_DUMP8("%02x ", convertedBtAddr, BT_ADDR_OUTPUT_PRINT_NUM);

    for (uint8_t devIndex = 0; devIndex < BT_DEVICE_NUM; devIndex++)
    {
        if (app_bt_get_device_bdaddr(devIndex, connectedBtAddr))
        {
            VOICEPATH_DUMP8("%02x ", connectedBtAddr, BT_ADDR_OUTPUT_PRINT_NUM);
