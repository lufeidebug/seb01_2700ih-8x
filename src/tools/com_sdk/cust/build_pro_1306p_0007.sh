...
APP_CUSTOMER_CFG="IGNORE_POWER_ON_KEY_DURING_BOOT_UP=1 IS_AUTOPOWEROFF_ENABLED=0 APP_TRACE_RX_ENABLE=1 APP_RX_API_ENABLE=1 AUDIO_DEBUG_CMD=0 \
                 TRACE_BUF_SIZE=20*1024 USE_TRACE_ID=0 BT_SVC_FW_PRODUCT=BT_SVC_FW_PRODUCT_HEADSET IBRT=0 IBRT_UI=0 APP_RSSI=0 AUDIO_SECTION_ENABLE=1"
...
BES_LIB_DIR="BES_LIB_DIR=lib/bes/best1306p/PRO_0007"
################################### end of var define ###################################

build_app_cmd="make T=$TARGET_LIST  $LEA_CFG $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $FREEMAN_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ANC_CFG $ROM_CFG $BES_LIB_DIR -j64"
