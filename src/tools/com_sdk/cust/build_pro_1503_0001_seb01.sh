...
POWER_ON_CFG="POWER_ON_ENTER_TWS_PAIRING_ENABLED=0 FREE_TWS_PAIRING_ENABLED=0"

AUDIO_CODEC_CFG="A2DP_AAC_ON=1 A2DP_LDAC_ON=0 A2DP_LHDC_ON=0 A2DP_LHDC_V3=0 A2DP_LHDCV5_ON=0"
...
then
    make T=$TARGET_LIST $BUILD_CUSTOMER_CFG $DEBUG_LIB_CFG -j64 lst all ||{ echo "$LINENO command failed"; exit 1; }
