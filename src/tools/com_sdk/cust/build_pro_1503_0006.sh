...
APP_CUSTOMER_CFG="IGNORE_POWER_ON_KEY_DURING_BOOT_UP=1 IS_AUTOPOWEROFF_ENABLED=0 APP_TRACE_RX_ENABLE=1 APP_RX_API_ENABLE=1 AUDIO_DEBUG_CMD=0 \
                 TRACE_BUF_SIZE=20*1024 USE_TRACE_ID=0 CAPSENSOR_ENABLE=1 DUAL_MIC_RECORDING=1 RECORDING_USE_OPUS=1 SINGLE_DEVICE_REC=1"

OTA_CFG="BES_OTA=1 OTA_BIN_COMPRESSED=0"

BLE_CFG="BLE=1 GATT_OVER_BR_EDR=1"

ALGO_CFG="A2DP_VIRTUAL_SURROUND=1 HEAD_TRACK_ENABLE=1 AUDIO_LIMITER=1 APP_DEBUG_TOOL=TOTA AUDIO_EQ_TUNING=1 AUDIO_ADJ_EQ_REV=1 ANC_ASSIST_ENABLED=1"
...
BES_LIB_DIR="BES_LIB_DIR=lib/bes/best1503/PRO_0006"
...
then
    make T=$TARGET_LIST $APP_CUSTOMER_CFG $OTA_CFG $BLE_CFG $ALGO_CFG $TWS_CFG $DECODE_DECODER_CFG $SYS_CFG $ROM_CFG $BES_LIB_DIR -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    if [[ $single_dac == 0 ]];
    then
        make T=$TARGET_LIST $APP_CUSTOMER_CFG $OTA_CFG $BLE_CFG $ALGO_CFG $FREEMAN_CFG $DECODE_DECODER_CFG $SYS_CFG $ROM_CFG $BES_LIB_DIR -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    fi
