...
BES_LIB_DIR="BES_LIB_DIR=lib/bes/best1503/PRO_0002"

# HM_FEAT="GFPS_ENABLE=1 SASS_ENABLED=0 SPOT_ENABLE=1 SWIFT_ENABLE=1 BLE_AUDIO_ENABLED=1 BIS_SELFSCAN_ENABLED=1 APP_BLE_BIS_SINK_ENABLE=1 APP_BLE_BIS_DELEG_ENABLE=1"

################################### end of var define ###################################

build_app_cmd="make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $TWS_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ANC_CFG $ROM_CFG -j64"
...
then
    make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $TWS_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ROM_CFG $BES_LIB_DIR -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $TWS_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ANC_CFG $ROM_CFG $BES_LIB_DIR -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    if [[ $single_dac == 0 ]];
    then
        make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $FREEMAN_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ROM_CFG $BES_LIB_DIR -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
        make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $FREEMAN_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ANC_CFG $ROM_CFG $BES_LIB_DIR -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    fi
