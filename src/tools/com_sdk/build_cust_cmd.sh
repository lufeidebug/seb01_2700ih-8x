...
then
    make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $TWS_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ROM_CFG -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $TWS_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ANC_CFG $ROM_CFG -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    if [[ $single_dac == 0 ]];
    then
        make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $FREEMAN_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ROM_CFG -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
        make T=$TARGET_LIST $APP_CUSTOMER_CFG $BLE_CFG $OTA_CFG $FREEMAN_CFG $SPEECH_CFG $DECODE_DECODER_CFG $SYS_CFG $ANC_CFG $ROM_CFG -j64 GEN_LIB=1 ||{ echo "$LINENO command failed"; exit 1; }
    fi
