...
#include "../../../../../utils/heap/custom_allocator.h"
...
#define ANC_PCM_BITS (16)
#define FREQ_SCALE (256)
#else
typedef float anc_assist_pcm_t;
#define ANC_PCM_BITS (32)
#define FREQ_SCALE (1)
#endif
...
typedef enum {
    ...
    ANC_ASSIST_ALGO_ID_ADAPTIVE_MODE    = (0x1 << 13),
...
typedef enum {
    NOISE_STATUS_QUIET_EXTREMELY_ANC = 0,
...
#define NOISE_DETECT_BAND_NUM   (3)
typedef struct {
    uint32_t    period_ms;
    float       smooth_factor;
    float       band_freq[NOISE_DETECT_BAND_NUM+1];
    float       band_weight[NOISE_DETECT_BAND_NUM];
    float       speech_detect_prod_th;
} assist_noise_detect_cfg_t;

#define NOISE_GAIN_MAPPING_MAX_NUM_NUM   (30)
typedef struct {
    float   smooth_factor;
    float   gain_db_update_min_diff;
    uint32_t noise_gain_mapping_num;
    float noise_db[NOISE_GAIN_MAPPING_MAX_NUM_NUM];
    float gain_db[NOISE_GAIN_MAPPING_MAX_NUM_NUM];
} assist_noise_gain_cfg_t;

typedef struct {
    bool    debug_en;
    assist_noise_detect_cfg_t noise_detect_cfg;
    assist_noise_gain_cfg_t noise_gain_cfg;
} assist_adaptive_mode_cfg_t;

typedef enum {
    ASSIST_ADAPTIVE_MODE_STATUS_NO_CHANGED = 0,
    ASSIST_ADAPTIVE_MODE_STATUS_CHANGED
} assist_adaptive_mode_status_t;

typedef struct {
    assist_adaptive_mode_status_t status;
    float noise_db;
    float gain_linear;
    float gain_db;
} assist_adaptive_mode_res_t;
...
typedef struct {
    ...
    float vpu_min_th;
    float vpu_max_th;
    float vpu_ratio_min_th;
    float ff_min_th1;
    float ff_min_th2;
    float ff_min_th3;
    float ff_ratio_min_th;
    float vpu_scale_factor;
    float ff_scale_factor;
    float ff_vpu_ratio_th1;
    float ff_vpu_ratio_th2;
    float cdr_th1;
    float cdr_th2;

    float ovad_conf_th1;
    float ovad_conf_th2;
    float ovad_ratio_th1;
    float ovad_ratio_th2;

    float tvad_conf_th;
    float tvad_ratio_th;

    uint32_t start_freq_index;
    uint32_t end_freq_index;

    uint32_t wsd_win_len;
    uint32_t wsd_out_len;
    uint32_t vpu_vad_out_len;
    uint32_t ai_vad_win_len;
    uint32_t ai_vad_out_len;
} assist_vpu_wsd_cfg_t;
...
typedef struct {
	...
	uint32_t 	adaptive_volume_en;
	uint32_t 	extern_adj_eq_rev_en;
	uint32_t    iir_lms_en;
    uint32_t    adaptive_mode_en;
    uint32_t    ai_vad_en;
	...
	assist_noise_detect_cfg_t	ada_volume_detect_cfg;
    ...
    assist_adaptive_mode_cfg_t adaptive_mode_cfg;
	assist_wind_single_mic_cfg_t wind_single_mic_cfg;
    assist_vpu_wsd_cfg_t    vpu_wsd_cfg;
} AncAssistConfig;
...
typedef struct {
	...
	// NOISE DETECTION
	float	ada_volume_noise_energy;
	float	ref_detect_energy;
	anc_assist_algo_status_t ada_volume_noise_status;
    ...
    assist_adaptive_mode_res_t adaptive_mode_res;
} AncAssistRes;
...
static POSSIBLY_UNUSED inline bool all_of_u32(uint32_t *status, uint32_t len, uint32_t expected_status)
{
	for (uint32_t i = 0; i < len; i++) {
		if (status[i] != expected_status)
			return false;
...
static POSSIBLY_UNUSED inline bool any_of_bool(bool *status, uint32_t len, bool expected_status)
{
	for (uint32_t i = 0; i < len; i++) {
		if (status[i] == expected_status)
			return true;
...
static POSSIBLY_UNUSED inline bool all_of_bool(bool *status, uint32_t len, bool expected_status)
{
	for (uint32_t i = 0; i < len; i++) {
		if (status[i] != expected_status)
			return false;
...
AncAssistRes anc_assist_process(AncAssistState *st, anc_assist_pcm_t **ff_mic, uint8_t ff_ch_num, anc_assist_pcm_t **fb_mic, uint8_t fb_ch_num, anc_assist_pcm_t **talk_mic, uint8_t talk_ch_num, anc_assist_pcm_t **ref, uint8_t ref_ch_num, anc_assist_pcm_t *vpu_mic, uint32_t frame_len);
...
uint32_t anc_assist_get_AncAssistConfig_size(void);
